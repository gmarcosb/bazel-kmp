...
internal object CentralDirectory {
    /**
     * Writes the central directory to an output stream for the specified [ZipFileData].
     */
    @Throws(IOException::class)
    fun write(file: ZipFileData, allowZip64: Boolean, stream: OutputStream) {
        var directorySize: Long = 0
        val buf = ByteArray(CentralDirectoryFileHeader.FIXED_DATA_SIZE)
        for (entry in file.getEntries()) {
            directorySize += CentralDirectoryFileHeader.write(entry, file, allowZip64, buf, stream).toLong()
        }
