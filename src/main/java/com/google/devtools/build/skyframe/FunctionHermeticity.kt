...
/**
 * Hermeticity of a [SkyFunction], meaning whether it accesses external state untracked by
 * Skyframe during its evaluation. A classic example is a [SkyFunction] that consumes a file
 ...
 * 
 * 
 * Note that Skyframe does *not* explicitly re-evaluate non-hermetic functions on every
 * build: it just relaxes some of its graph-pruning logic to be more conservative with such nodes.
 */
enum class FunctionHermeticity {
    /**
     ...
     * 
     * 
     * Nodes produced by this type of function are treated the same as HERMETIC with respect to
