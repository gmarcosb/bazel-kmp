// Copyright 2022 The Bazel Authors. All rights reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//    http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
package com.google.devtools.build.lib.profiler

import com.google.devtools.build.lib.profiler.TraceData
import com.google.gson.stream.JsonWriter
import java.io.IOException

/** This class is used to insert metadata about threads into the JSON trace profile.  */
internal class ThreadMetadata : TraceData {
    private val readableName: String?
    private val threadId: Long
    private val sortIndex: Long

    constructor(readableName: String, threadId: Long) {
        this.readableName = readableName
        this.threadId = threadId
        this.sortIndex = getSortIndex(readableName)
    }

    constructor(readableName: String?, threadId: Long, sortIndex: Long) {
        this.readableName = readableName
        this.threadId = threadId
        this.sortIndex = sortIndex
    }

    constructor() {
        this.readableName = getReadableName(java.lang.Thread.currentThread().getName())
        this.threadId = java.lang.Thread.currentThread().getId()
        this.sortIndex = getSortIndex(java.lang.Thread.currentThread().getName())
    }

    @Throws(IOException::class)
    override fun writeTraceData(jsonWriter: JsonWriter, profileStartTimeNanos: Long) {
        jsonWriter.setIndent("  ")
        jsonWriter.beginObject()
        jsonWriter.setIndent("")
        jsonWriter.name("name").value("thread_name")
        jsonWriter.name("ph").value("M")
        jsonWriter.name("pid").value(1)
        jsonWriter.name("tid").value(threadId)
        jsonWriter.name("args")

        jsonWriter.beginObject()
        jsonWriter.name("name").value(readableName)
        jsonWriter.endObject()

        jsonWriter.endObject()

        jsonWriter.setIndent("  ")
        jsonWriter.beginObject()
        jsonWriter.setIndent("")
        jsonWriter.name("name").value("thread_sort_index")
        jsonWriter.name("ph").value("M")
        jsonWriter.name("pid").value(1)
        jsonWriter.name("tid").value(threadId)
        jsonWriter.name("args")

        jsonWriter.beginObject()
        jsonWriter.name("sort_index").value(sortIndex)
        jsonWriter.endObject()

        jsonWriter.endObject()
    }

    companion object {
        // The JVM never returns 0 as thread ID, so we use that as fake thread ID for the critical path.
        const val CRITICAL_PATH_THREAD_ID: Long = 0

        // In order to display the actions on the critical path in a nice way, we fake a thread for them.
        fun createFakeThreadMetadataForCriticalPath(): ThreadMetadata {
            return ThreadMetadata("Critical Path", CRITICAL_PATH_THREAD_ID, CRITICAL_PATH_SORT_INDEX)
        }

        private fun isMainThread(threadName: String): Boolean {
            return threadName.startsWith("grpc-command")
        }

        private fun isGCThread(threadName: String): Boolean {
            return threadName == "Notification Thread"
        }

        private fun getReadableName(threadName: String): String {
            if (isMainThread(threadName)) {
                return "Main Thread"
            }

            if (isGCThread(threadName)) {
                return "Garbage Collector"
            }

            return threadName
        }

        /**
         * These constants describe ranges of threads. We suppose that there are no more than 10_000
         * threads of each kind, otherwise the profile becomes unreadable anyway. So the sort index of
         * skyframe threads is in range [10_000..20_000) for example.
         */
        private const val SKYFRAME_EVALUATOR_SHIFT: Long = 10000

        private const val SKYFRAME_EVALUATOR_CPU_HEAVY_SHIFT: Long = 20000
        private const val SKYFRAME_EVALUATOR_EXECUTION_SHIFT: Long = 30000
        private const val SKYFRAME_EVALUATOR_MEMOIZING_SHIFT: Long = 40000

        private const val DYNAMIC_EXECUTION_SHIFT: Long = 100000
        private const val INCLUDE_SCANNER_SHIFT: Long = 200000

        private const val CRITICAL_PATH_SORT_INDEX: Long = 0
        private const val MAIN_THREAD_SORT_INDEX: Long = 1
        private const val GC_THREAD_SORT_INDEX: Long = 2
        private const val MAX_SORT_INDEX: Long = 1000000

        private val NUMBER_PATTERN: java.util.regex.Pattern = java.util.regex.Pattern.compile("\\d+")

        private fun getSortIndex(threadName: String): Long {
            if (isMainThread(threadName)) {
                return MAIN_THREAD_SORT_INDEX
            }

            if (isGCThread(threadName)) {
                return GC_THREAD_SORT_INDEX
            }

            val numberMatcher: java.util.regex.Matcher = NUMBER_PATTERN.matcher(threadName)
            if (!numberMatcher.find()) {
                return MAX_SORT_INDEX
            }

            val extractedNumber: Long
            try {
                extractedNumber = java.lang.Long.parseLong(numberMatcher.group())
            } catch (e: java.lang.NumberFormatException) {
                // If the number cannot be parsed, e.g. is larger than a long, the actual position is not
                // really relevant.
                return MAX_SORT_INDEX
            }

            if (threadName.startsWith("skyframe-evaluator")) {
                if (threadName.startsWith("skyframe-evaluator-cpu-heavy")) {
                    return SKYFRAME_EVALUATOR_CPU_HEAVY_SHIFT + extractedNumber
                }
                if (threadName.startsWith("skyframe-evaluator-execution")) {
                    return SKYFRAME_EVALUATOR_EXECUTION_SHIFT + extractedNumber
                }
                if (threadName.startsWith("skyframe-evaluator-memoizing")) {
                    return SKYFRAME_EVALUATOR_MEMOIZING_SHIFT + extractedNumber
                }
                return SKYFRAME_EVALUATOR_SHIFT + extractedNumber
            }

            if (threadName.startsWith("dynamic-execution")) {
                return DYNAMIC_EXECUTION_SHIFT + extractedNumber
            }

            if (threadName.startsWith("Include scanner")) {
                return INCLUDE_SCANNER_SHIFT + extractedNumber
            }

            return MAX_SORT_INDEX
        }
    }
}
