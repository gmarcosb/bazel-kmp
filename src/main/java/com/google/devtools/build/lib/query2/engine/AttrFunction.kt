 ...
 */
class AttrFunction : RegexFilterExpression {
    internal constructor() : super( /*invert=*/false)

    private constructor(invert: Boolean) : super(invert)

    override fun invert(): FilteringQueryFunction {
        return AttrFunction(!invert)
    }

    override fun getName(): String {
        return (if (invert) "no" else "") + "attr"
    }
    ...
    override fun <T> getFilterStrings(
        env: QueryEnvironment<T?>,
        args: MutableList<QueryEnvironment.Argument?>, target: T?
    ): Iterable<String?>? {
