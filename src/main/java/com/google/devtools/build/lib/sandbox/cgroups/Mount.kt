 ...
 */
class Mount(path: java.nio.file.Path?, type: String?, opts: com.google.common.collect.ImmutableList<String?>?) {
    val isV2: Boolean
        get() = this.type == "cgroup2"

    val path: java.nio.file.Path?
    val type: String?
    val opts: com.google.common.collect.ImmutableList<String?>?
    ...
    init {
        ...
        java.util.Objects.requireNonNull<String?>(type, "type")
        java.util.Objects.requireNonNull<com.google.common.collect.ImmutableList<String?>?>(opts, "opts")
    }
    ...
    companion object {
        ...
        private val CGROUPS_MOUNT_PATTERN: java.util.regex.Pattern =
            java.util.regex.Pattern.compile("^[^\\s#]\\S*\\s+(?<file>\\S*)\\s+(?<vfstype>cgroup2?)\\s+(?<mntops>\\S*).*")

        fun create(
            path: java.nio.file.Path?,
            type: String?,
            opts: com.google.common.collect.ImmutableList<String?>?
        ): Mount {
            return Mount(path, type, opts)
        }

        /**
         * Parses the cgroup mounts from the provided file and returns a list of mounts.
         * 
         * @param procMounts a file containing the cgroup mounts, typically `/proc/mounts`.
         * @return The list of cgroup mounts in the file.
         */
        @Throws(IOException::class)
        fun parse(procMounts: java.io.File): com.google.common.collect.ImmutableList<Mount?> {
            val mounts: com.google.common.collect.ImmutableList.Builder<Mount?> =
                com.google.common.collect.ImmutableList.builder<Mount?>()

            for (mount in com.google.common.io.Files.readLines(procMounts, java.nio.charset.StandardCharsets.UTF_8)) {
                val m: java.util.regex.Matcher = CGROUPS_MOUNT_PATTERN.matcher(mount)
                if (!m.matches()) {
                    continue
                }

                val path: String? = m.group("file")
                val type: String? = m.group("vfstype")
                val opts: com.google.common.collect.ImmutableList<String?> =
                    com.google.common.collect.ImmutableList.copyOf<String?>(
                        m.group("mntops").split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                    )
                mounts.add(create(Paths.get(path), type, opts))
            }
            return mounts.build()
        }
    }
