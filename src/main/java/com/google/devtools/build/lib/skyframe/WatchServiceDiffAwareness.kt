 ...
 */
class WatchServiceDiffAwareness internal constructor(
     ...
     */
    private val watchKeyToDirBiMap: com.google.common.collect.HashBiMap<WatchKey?, java.nio.file.Path?> =
        com.google.common.collect.HashBiMap.create<WatchKey?, java.nio.file.Path?>()
    ...
    private fun init() {
        com.google.common.base.Preconditions.checkState(watchService == null)
        try {
    ...
    public override fun getCurrentView(options: com.google.devtools.common.options.OptionsProvider): View? {
        ...
        val watchFs =
            options.getOptions<com.google.devtools.build.lib.skyframe.LocalDiffAwareness.Options?>(com.google.devtools.build.lib.skyframe.LocalDiffAwareness.Options::class.java)
                .getWatchFS()
                    ...
                    (!isWindows || options.getOptions<com.google.devtools.build.lib.skyframe.LocalDiffAwareness.Options?>(
        ...
        val modifiedAbsolutePaths: MutableSet<java.nio.file.Path?>?
        if (isFirstCall()) {
            try {
            ...
            modifiedAbsolutePaths = com.google.common.collect.ImmutableSet.of<java.nio.file.Path?>()
        } else {
            try {
    ...
    @Throws(BrokenDiffAwarenessException::class, IOException::class)
    private fun collectChanges(): MutableSet<java.nio.file.Path?> {
        val createdFilesAndDirectories: MutableSet<java.nio.file.Path?> = HashSet<java.nio.file.Path?>()
        val deletedOrModifiedFilesAndDirectories: MutableSet<java.nio.file.Path?> = HashSet<java.nio.file.Path?>()
        val deletedTrackedDirectories: MutableSet<java.nio.file.Path?> = HashSet<java.nio.file.Path?>()

        var watchKey: WatchKey?
        while ((watchService.poll().also { watchKey = it }) != null) {
            val dir: java.nio.file.Path? = watchKeyToDirBiMap.get(watchKey)
            com.google.common.base.Preconditions.checkArgument(dir != null)
            ...
            for (event in watchKey.pollEvents()) {
                ...
                val relativePath: java.nio.file.Path? = event.context() as java.nio.file.Path?
                val path: java.nio.file.Path = dir.resolve(relativePath)
                com.google.common.base.Preconditions.checkState(path.isAbsolute(), path)
                if (kind === StandardWatchEventKinds.ENTRY_CREATE) {
                    ...
                    val deletedDirectoryKey: WatchKey? = watchKeyToDirBiMap.inverse().get(path)
        ...
        for (path in deletedTrackedDirectories) {
            val staleKey: WatchKey? = watchKeyToDirBiMap.inverse().get(path)
        ...
        if (watchKeyToDirBiMap.isEmpty()) {
            ...
            throw IOException("Root directory " + watchRoot + " became inaccessible.")
        }

        val changedPaths: MutableSet<java.nio.file.Path?> = HashSet<java.nio.file.Path?>()
        for (path in createdFilesAndDirectories) {
    ...
    private fun registerSubDirectories(rootDir: java.nio.file.Path?) {
        // Note that this does not follow symlinks.
        val watcherFileVisitor = WatcherFileVisitor(ignoredPaths)
        java.nio.file.Files.walkFileTree(rootDir, watcherFileVisitor)
    }
     ...
     */
    @Throws(IOException::class)
    private fun registerSubDirectoriesAndReturnContents(rootDir: java.nio.file.Path?): MutableSet<java.nio.file.Path?> {
        val visitedAbsolutePaths: MutableSet<java.nio.file.Path?> = HashSet<java.nio.file.Path?>()
        // Note that this does not follow symlinks.
        val watcherFileVisitor =
            WatcherFileVisitor(visitedAbsolutePaths, ignoredPaths)
        java.nio.file.Files.walkFileTree(rootDir, watcherFileVisitor)
        return visitedAbsolutePaths
    }

    /** File visitor used by Files.walkFileTree() upon traversing subdirectories.  */
    private inner class WatcherFileVisitor : SimpleFileVisitor<java.nio.file.Path?> {
        private val visitedAbsolutePaths: MutableSet<java.nio.file.Path?>
        private val ignoredPaths: IgnoredSubdirectories

        private constructor(visitedPaths: MutableSet<java.nio.file.Path?>, ignoredPaths: IgnoredSubdirectories) {
            this.visitedAbsolutePaths = visitedPaths
            this.ignoredPaths = ignoredPaths
        }

        private constructor(ignoredPaths: IgnoredSubdirectories) {
            this.visitedAbsolutePaths = HashSet<java.nio.file.Path?>()
            this.ignoredPaths = ignoredPaths
        }
        ...
        @Throws(IOException::class)
        override fun preVisitDirectory(path: java.nio.file.Path, attrs: BasicFileAttributes): FileVisitResult {
