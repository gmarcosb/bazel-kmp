...
@StarlarkBuiltin(
    name = "CcInfo", category = DocCategory.PROVIDER, doc = ("A provider for compilation and linking of C++. This "
...
interface CcInfoApi<FileT : FileApi?> : StructApi {
    @get:StarlarkMethod(
        name = "compilation_context",
        doc = "Returns the <code>CompilationContext</code>",
        structField = true
    )
    val ccCompilationContext: CcCompilationContextApi<FileT?>?
    ...
    val ccLinkingContextForStarlark: StarlarkInfo?
    ...
    val ccTransitiveNativeLibraries: Depset?
    ...
    interface Provider<FileT : FileApi?> : ProviderApi {
        @StarlarkMethod(
            ...
            parameters = [Param(
                ...
                named = true,
                defaultValue = "None",
                allowedTypes = [ParamType(type = CcCompilationContextApi::class), ParamType(type = NoneType::class)]
            ), Param(
                name = "linking_context",
        ...
        fun createInfo(
            ccCompilationContext: Any?,
            ccLinkingInfo: Any?,
            ccDebugInfo: Any?,
            ccNativeLibraryInfo: Any?,
            thread: StarlarkThread?
        ): CcInfoApi<FileT?>?
    }

    companion object {
        const val NAME: String = "CcInfo"
    }
}
