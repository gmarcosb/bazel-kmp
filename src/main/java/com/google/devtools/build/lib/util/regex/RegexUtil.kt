...
object RegexUtil {
    ...
    private val LITERAL_PATTERN_WITH_DOT_UNESCAPED: Pattern = Pattern.compile("(?:[^\\[\\](){}^$|*+?.\\\\]|\\\\\\.)*")
    ...
    private val EXTRACT_SUFFIX_MATCH: Pattern = Pattern.compile("(?:\\^|\\\\A)*\\.\\*(?<suffix>[^?+].*)?")

    /**
     * Returns a [Predicate] that matches the input string (in internal encoding according to
     * [StringEncoding]) against the regex pattern with the
     * same semantics as [Matcher.matches], but more optimized and as if the pattern was
     * compiled with [Pattern.DOTALL].
     */
    @kotlin.jvm.JvmStatic
    fun asOptimizedMatchingPredicate(regexPattern: Pattern): Predicate<String?> {
        val pattern = regexPattern.pattern()
        if (pattern.contains("|") || pattern.contains("\\Q")) {
            ...
            return Predicate { s: String? -> regexPattern.matcher(StringEncoding.internalToUnicode(s)).matches() }
        }
        ...
        if (LITERAL_PATTERN_WITH_DOT_UNESCAPED.matcher(pattern).matches()) {
            return Predicate { anObject: String? ->
                StringEncoding.unicodeToInternal(pattern.replace("\\.", ".")).equals(anObject)
            }
        }
        ...
        val suffixMatch = EXTRACT_SUFFIX_MATCH.matcher(pattern)
        if (!suffixMatch.matches()) {
            return Predicate { s: String? -> regexPattern.matcher(StringEncoding.internalToUnicode(s)).matches() }
        }
        val suffixPattern = suffixMatch.group("suffix")
        // A null suffixPattern implies the regex is equivalent to ".*", which matches any input string.
        if (suffixPattern == null) {
            return Predicate { s: String? -> true }
        }
        ...
        if (LITERAL_PATTERN_WITH_DOT_UNESCAPED.matcher(suffixPattern).matches()) {
            val literalSuffix = StringEncoding.unicodeToInternal(suffixPattern.replace("\\.", "."))
            return Predicate { s: String? -> s.endsWith(literalSuffix) }
        }
        ...
        val compiled = Pattern.compile(suffixPattern + "\\z", Pattern.DOTALL)
        return Predicate { s: String? -> compiled.matcher(StringEncoding.internalToUnicode(s)).find() }
    }
}
