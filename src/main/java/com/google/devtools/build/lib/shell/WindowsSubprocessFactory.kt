...
class WindowsSubprocessFactory : SubprocessFactory {
    @Throws(IOException::class)
    override fun create(builder: SubprocessBuilder): Subprocess {
        com.google.common.base.Preconditions.checkState(com.google.devtools.build.lib.util.OS.getCurrent() == com.google.devtools.build.lib.util.OS.WINDOWS)
        val argv: MutableList<String?> = com.google.common.collect.Lists.transform<String?, String?>(
            builder.getArgv(),
        ...
        val argv0 = Companion.processArgv0(argv.get(0)!!)
        val argvRest =
            if (argv.size > 1)
                Companion.escapeArgvRest(argv.subList(1, argv.size), argv0 == "cmd.exe")
            else
                ""
        val env = convertEnvToNative(builder.getEnv(), builder.getClientEnv())

        var cwd: String? = null
        if (builder.getWorkingDirectory() != null) {
            cwd = builder.getWorkingDirectory().getPath()
        }
        val stdoutPath = getRedirectPath(builder.getStdout(), builder.getStdoutFile())
        val stderrPath = getRedirectPath(builder.getStderr(), builder.getStderrFile())

        val nativeProcess: Long =
            WindowsProcesses.createProcess(
                argv0, argvRest, env, cwd, stdoutPath, stderrPath, builder.redirectErrorStream()
            )
        val error: String = WindowsProcesses.processGetLastError(nativeProcess)
        if (!error.isEmpty()) {
            WindowsProcesses.deleteProcess(nativeProcess)
            throw IOException(error)
        }

        return WindowsSubprocess(
            nativeProcess,
            argv0 + " " + argvRest,
            stdoutPath != null,
            stderrPath != null,
    ...
    companion object {
        @kotlin.jvm.JvmField
        val INSTANCE: WindowsSubprocessFactory = WindowsSubprocessFactory()

        @kotlin.jvm.JvmStatic
        fun maybeInstallWindowsSubprocessFactory() {
            if (com.google.devtools.build.lib.util.OS.getCurrent() == com.google.devtools.build.lib.util.OS.WINDOWS) {
                SubprocessBuilder.Companion.setDefaultSubprocessFactory(INSTANCE)
            }
        }

        private fun escapeArgvRest(argv: MutableList<String>, isCmd: Boolean): String {
        ...
        fun processArgv0(argv0: String): String {
            ...
            val argv0fragment: PathFragment = PathFragment.create(argv0)
            return if (argv0fragment.isAbsolute()) argv0fragment.getPathString().replace('/', '\\') else argv0
        }

        private fun getRedirectPath(action: StreamAction, file: java.io.File): String? {
            return when (action) {
                StreamAction.DISCARD -> "NUL"
                StreamAction.REDIRECT -> file.getPath()
                StreamAction.STREAM -> null
            }
        }

        /** Converts an environment map to the format expected in lpEnvironment by CreateProcess().  */
        private fun convertEnvToNative(
            envMap: MutableMap<String?, String?>?, clientEnv: com.google.common.collect.ImmutableMap<String?, String?>
        ): ByteArray? {
            val fullEnv: MutableMap<String?, String?> =
                TreeMap<String?, String?>(java.lang.String.CASE_INSENSITIVE_ORDER)
            ...
            if (envMap != null) {
                ...
                for (env in com.google.common.collect.ImmutableList.of<String?>("SYSTEMROOT", "SYSTEMDRIVE")) {
                    if (fullEnv.getOrDefault(env, null) == null) {
                        val value: String? = clientEnv.get(env)
                        if (value != null) {
            ...
            if (fullEnv.isEmpty()) {
                ...
                // with four zeros.
                return "\u0000\u0000".toByteArray(java.nio.charset.StandardCharsets.UTF_16LE)
            }

            val result: java.lang.StringBuilder = java.lang.StringBuilder()
            for (entry in fullEnv.entries) {
                ...
                result
                    .append(StringEncoding.internalToPlatform(entry.key))
                    .append("=")
                    .append(StringEncoding.internalToPlatform(entry.value))
                    .append("\u0000")
            }

            result.append("\u0000")
            return result.toString().toByteArray(java.nio.charset.StandardCharsets.UTF_16LE)
        }
    }
}
