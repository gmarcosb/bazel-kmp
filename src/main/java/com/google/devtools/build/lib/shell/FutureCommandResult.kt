 ...
 */
class FutureCommandResult internal constructor(
    command: com.google.devtools.build.lib.shell.Command?,
    process: Subprocess,
    outErrConsumers: OutErrConsumers,
    killSubprocessOnInterrupt: Boolean
) {
    private val command: com.google.devtools.build.lib.shell.Command?
    private val process: Subprocess
    private val outErrConsumers: OutErrConsumers
    private val killSubprocessOnInterrupt: Boolean
     ...
     * [.isDone], the call will block until that process terminates.
     * 
     * @return non-null result of command execution
     * @throws AbnormalTerminationException if command execution failed
     * @throws InterruptedException if [.killSubprocessOnInterrupt] is true and thread is
     * interrupted before subprocess completes.
     */
    @Throws(AbnormalTerminationException::class, java.lang.InterruptedException::class)
    fun get(): CommandResult {
        ...
        try {
            ...
            val noOutputResult: CommandResult =
                CommandResult.Companion.builder()
                    .setStdoutStream(CommandResult.Companion.EMPTY_OUTPUT)
                    .setStderrStream(CommandResult.Companion.EMPTY_OUTPUT)
                    .setTerminationStatus(status)
                    .build()
            if (status.success()) {
                // If command was otherwise successful, throw an exception about this
                throw AbnormalTerminationException(command, noOutputResult, ioe)
            } else {
                // Otherwise, throw the more important exception -- command
                // was not successful
                val message = status.toString() + "; also encountered an error while attempting to retrieve output"
                throw if (status.exited())
                    BadExitStatusException(command, noOutputResult, message, ioe)
                else
                    AbnormalTerminationException(command, noOutputResult, message, ioe)
            }
        } finally {
            process.close()
        }
        ...
        if (status.success()) {
            ...
            throw BadExitStatusException(command, commandResult, status.toString())
        } else {
            throw AbnormalTerminationException(command, commandResult, status.toString())
        }
    }
    ...
    companion object {
        ...
        private fun waitForProcess(
            process: Subprocess, killSubprocessOnInterrupt: Boolean
        ): TerminationStatus {
            ...
            try {
                ...
                return TerminationStatus(process.exitValue(), process.timedout())
            } finally {
