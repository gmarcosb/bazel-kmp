// Copyright 2014 The Bazel Authors. All rights reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//    http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
package com.google.devtools.build.lib.exec

import com.google.devtools.build.lib.actions.Artifact

/**
 * Helper class responsible for the symlink tree creation. Used to generate runfiles and fileset
 * symlink farms.
 */
class SymlinkTreeHelper(
    inputManifest: com.google.devtools.build.lib.vfs.Path,
    outputManifest: com.google.devtools.build.lib.vfs.Path,
    symlinkTreeRoot: com.google.devtools.build.lib.vfs.Path,
    private val workspaceName: String?
) {
    private val inputManifest: com.google.devtools.build.lib.vfs.Path
    private val outputManifest: com.google.devtools.build.lib.vfs.Path
    private val symlinkTreeRoot: com.google.devtools.build.lib.vfs.Path

    /**
     * Creates SymlinkTreeHelper instance. Can be used independently of SymlinkTreeAction.
     * 
     * @param inputManifest exec path to the input runfiles manifest
     * @param outputManifest path to the output runfiles manifest
     * @param symlinkTreeRoot the root of the symlink tree to be created
     * @param workspaceName the name of the workspace, used to create the workspace subdirectory
     */
    init {
        // Do not indirect paths through overlay filesystems (such as the action filesystem or a
        // snapshotting filesystem), for a few reasons:
        // (1) we always want to create the symlinks on disk, even if the overlay filesystem creates
        //     them in memory (at the time of writing, no action filesystem implementations do so, but
        //     this may change in the future).
        // (2) current action filesystem implementations are not a true overlay filesystem, so errors
        //     might occur in an incremental build when the parent directory of a symlink exists on disk
        //     but not in memory (see https://github.com/bazelbuild/bazel/issues/24867).
        // (3) Changes made to a file referenced by a symlink tree should be reflected in the symlink
        //     tree without having to rebuild. Therefore, if a snapshotting file system is used, we must
        //     use the underlying non-snapshotting file system instead to create the symlink tree.
        this.inputManifest = inputManifest.forHostFileSystem()
        this.outputManifest = outputManifest.forHostFileSystem()
        this.symlinkTreeRoot = symlinkTreeRoot.forHostFileSystem()
    }

    internal interface TargetPathFunction<T> {
        /** Obtains a symlink target path from a T.  */
        @Throws(IOException::class)
        fun get(target: T?): PathFragment?
    }

    /** Creates a symlink tree for a fileset by making VFS calls.  */
    @Throws(ExecException::class)
    fun createFilesetSymlinks(symlinkMap: MutableMap<PathFragment?, PathFragment?>) {
        createSymlinks<PathFragment?>(symlinkMap, TargetPathFunction { path: PathFragment? -> path })
    }

    /** Creates a symlink tree for a runfiles by making VFS calls.  */
    @Throws(ExecException::class)
    fun createRunfilesSymlinks(symlinks: MutableMap<PathFragment?, Artifact?>) {
        createSymlinks<Artifact?>(
            symlinks,
            TargetPathFunction { artifact: Artifact? ->
                if (artifact.isSymlink() // Unresolved symlinks are created textually.
                )
                    artifact.getPath().readSymbolicLink()
                else
                    artifact.getPath().asFragment()
            })
    }

    /** Creates a symlink tree.  */
    @Throws(ExecException::class)
    private fun <T> createSymlinks(
        symlinkMap: MutableMap<PathFragment?, T?>, targetPathFn: TargetPathFunction<T?>
    ) {
        // Our strategy is to minimize mutating file system operations as much as possible. Ideally, if
        // there is an existing symlink tree with the expected contents, we don't make any changes. Our
        // algorithm goes as follows:
        //
        // 1. Create a tree structure that represents the entire set of paths that we want to exist. The
        //    tree structure contains a node for every intermediate directory. For example, this is the
        //    tree structure corresponding to the symlinks {"a/b/c": "foobar", "a/d/e": null}:
        //
        //      / b - c (symlink to "foobar")
        //    a
        //      \ d - e (empty file)
        //
        //    Note that we need to distinguish directories, symlinks, and empty files. In the Directory
        //    class below, we use two maps for that purpose: one for directories, and one for symlinks
        //    and empty files. This avoids having to create additional classes / objects to distinguish
        //    them.
        //
        // 2. Perform a depth-first traversal over the on-disk file system tree and make each directory
        //    match our expected directory layout. To that end, call readdir, and compare the result
        //    with the contents of the corresponding node in the in-memory tree.
        //
        //    For each Dirent entry in the readdir result:
        //    - If the entry is not in the current node, if the entry has an incompatible type, or if it
        //      is a symlink that points to the wrong location, delete the entry on disk (recursively).
        //    - Otherwise:
        //      - If the entry is a directory, recurse into that directory
        //      - In all cases, delete the entry in the current in-memory node.
        //
        // 3. For every remaining entry in the node, create the corresponding file, symlink, or
        //    directory on disk. If it is a directory, recurse into that directory.
        try {
            com.google.devtools.build.lib.profiler.Profiler.instance().profile("Create symlink tree").use { c ->
                val root: Directory<T?> = com.google.devtools.build.lib.exec.SymlinkTreeHelper.Directory<T?>()
                for (entry in symlinkMap.entrySet()) {
                    val path: PathFragment = entry.getKey()
                    val value: T? = entry.getValue()
                    com.google.common.base.Preconditions.checkArgument(!path.isEmpty() && !path.isAbsolute(), path)
                    // This creates intermediate directory nodes as a side effect.
                    val parentDir = root.walk(path.getParentDirectory())
                    parentDir.addSymlink(path.getBaseName(), value)
                }
                root.syncTreeRecursively(symlinkTreeRoot, targetPathFn)
                createWorkspaceSubdirectory()
            }
        } catch (e: IOException) {
            throw EnvironmentalExecException(e, Code.SYMLINK_TREE_CREATION_IO_EXCEPTION)
        }
    }

    /**
     * Creates a minimal runfiles directory containing only the output manifest and the workspace
     * subdirectory. This is the expected state with --noenable_runfiles or --nobuild_runfile_links.
     */
    @Throws(ExecException::class)
    fun createMinimalRunfilesDirectory() {
        clearRunfilesDirectory()
        linkManifest()
        createWorkspaceSubdirectory()
    }

    /** Deletes the contents of the runfiles directory.  */
    @Throws(ExecException::class)
    private fun clearRunfilesDirectory() {
        try {
            com.google.devtools.build.lib.profiler.Profiler.instance().profile("Clear symlink tree").use { c ->
                symlinkTreeRoot.deleteTreesBelow()
            }
        } catch (e: IOException) {
            throw EnvironmentalExecException(e, Code.SYMLINK_TREE_DELETION_IO_EXCEPTION)
        }
    }

    /** Links the output manifest to the input manifest.  */
    @Throws(ExecException::class)
    fun linkManifest() {
        // TODO(b/443703909): This should probably be a copy, not a symlink.
        try {
            outputManifest.delete()
            outputManifest.createSymbolicLink(inputManifest)
        } catch (e: IOException) {
            throw EnvironmentalExecException(e, Code.SYMLINK_TREE_MANIFEST_LINK_IO_EXCEPTION)
        }
    }

    @Throws(ExecException::class)
    fun createWorkspaceSubdirectory() {
        // Always create the subdirectory corresponding to the workspace (i.e., the main repository).
        // This is required by tests as their working directory, even with --noenable_runfiles. But if
        // the test action creates the directory and then proceeds to execute the test spawn, this logic
        // would remove it. For the sake of consistency, always create the directory instead.
        try {
            symlinkTreeRoot.getRelative(workspaceName).createDirectory()
        } catch (e: IOException) {
            throw EnvironmentalExecException(e, Code.SYMLINK_TREE_CREATION_IO_EXCEPTION)
        }
    }

    private class Directory<T> {
        private val symlinks: MutableMap<String?, T?> = HashMap<String?, T?>()
        private val directories: MutableMap<String?, Directory<T?>> = HashMap<String?, Directory<T?>>()

        fun addSymlink(basename: String?, target: T?) {
            symlinks.put(basename, target)
        }

        fun walk(dir: PathFragment): Directory<T?> {
            var result: Directory<T?> = this
            for (segment in dir.segments()) {
                result = result.directories.computeIfAbsent(
                    segment,
                    java.util.function.Function { unused: String? -> com.google.devtools.build.lib.exec.SymlinkTreeHelper.Directory<T?>() })
            }
            return result
        }

        @Throws(IOException::class)
        fun syncTreeRecursively(at: com.google.devtools.build.lib.vfs.Path, targetPathFn: TargetPathFunction<T?>) {
            val stat: FileStatus? = at.statIfFound(Symlinks.FOLLOW)
            if (stat == null) {
                at.createDirectoryAndParents()
            } else if (!stat.isDirectory()) {
                at.deleteTree()
                at.createDirectoryAndParents()
            } else {
                // If the directory already exists, ensure it has appropriate permissions.
                val perms: Int = stat.getPermissions()
                if (perms == -1) {
                    at.chmod(493)
                } else if ((perms and 448) != 448) {
                    at.chmod(stat.getPermissions() or 448)
                }
            }
            for (dirent in at.readdir(Symlinks.NOFOLLOW)) {
                val basename: String = dirent.getName()
                val next: com.google.devtools.build.lib.vfs.Path = at.getChild(basename)
                if (symlinks.containsKey(basename)) {
                    val value = symlinks.remove(basename)
                    if (value == null) {
                        if (dirent.getType() != com.google.devtools.build.lib.vfs.Dirent.Type.FILE) {
                            next.deleteTree()
                            com.google.devtools.build.lib.vfs.FileSystemUtils.createEmptyFile(next)
                        }
                        // For historical reasons, we don't truncate the file if one exists.
                        // TODO(tjgq): Ponder whether this is still necessary to preserve the intentional
                        // non-hermeticity of symlink trees under source edits.
                    } else {
                        // ensureSymbolicLink will replace a symlink that doesn't have the correct target, but
                        // everything else needs to be deleted first.
                        if (dirent.getType() != com.google.devtools.build.lib.vfs.Dirent.Type.SYMLINK) {
                            next.deleteTree()
                        }
                        com.google.devtools.build.lib.vfs.FileSystemUtils.ensureSymbolicLink(
                            next,
                            targetPathFn.get(value)
                        )
                    }
                } else if (directories.containsKey(basename)) {
                    val nextDir: Directory<T?> = directories.remove(basename)!!
                    if (dirent.getType() != com.google.devtools.build.lib.vfs.Dirent.Type.DIRECTORY) {
                        next.deleteTree()
                    }
                    nextDir.syncTreeRecursively(at.getChild(basename), targetPathFn)
                } else {
                    at.getChild(basename).deleteTree()
                }
            }

            for (entry in symlinks.entrySet()) {
                val next: com.google.devtools.build.lib.vfs.Path = at.getChild(entry.getKey())
                val value: T? = entry.getValue()
                if (value == null) {
                    com.google.devtools.build.lib.vfs.FileSystemUtils.createEmptyFile(next)
                } else {
                    com.google.devtools.build.lib.vfs.FileSystemUtils.ensureSymbolicLink(next, targetPathFn.get(value))
                }
            }
            for (entry in directories.entrySet()) {
                entry.getValue().syncTreeRecursively(at.getChild(entry.getKey()), targetPathFn)
            }
        }
    }

    companion object {
        /**
         * Processes a list of fileset symlinks into a map that can be passed to [ ][com.google.devtools.build.lib.vfs.OutputService.createSymlinkTree].
         * 
         * 
         * By convention, all symlinks are placed under a directory with the given workspace name.
         */
        fun processFilesetLinks(
            links: com.google.common.collect.ImmutableList<FilesetOutputSymlink>, workspaceName: String?
        ): com.google.common.collect.ImmutableMap<PathFragment?, PathFragment?> {
            val root: PathFragment = PathFragment.create(workspaceName)
            val symlinks: com.google.common.collect.ImmutableMap.Builder<PathFragment?, PathFragment?> =
                com.google.common.collect.ImmutableMap.builderWithExpectedSize<PathFragment?, PathFragment?>(links.size())
            for (symlink in links) {
                symlinks.put(root.getRelative(symlink.name()), symlink.target().getPath().asFragment())
            }
            // Fileset links are already deduplicated by name in SkyframeFilesetManifestAction.
            return symlinks.buildOrThrow()
        }
    }
}
