...
object ExecutionStatistics {
    /**
     * Provides execution statistics based on a `execution_statistics.proto` file.
     * 
     * @param executionStatisticsProtoPath path to a materialized ExecutionStatistics proto
     * @return a [ResourceUsage] object containing execution statistics, if available
     */
    @Throws(IOException::class)
    fun getResourceUsage(executionStatisticsProtoPath: com.google.devtools.build.lib.vfs.Path): java.util.Optional<ResourceUsage?> {
        try {
            BufferedInputStream(executionStatisticsProtoPath.getInputStream()).use { protoInputStream ->
                val executionStatisticsProto: ExecutionStatistics =
                    Protos.ExecutionStatistics.parseFrom(protoInputStream)
                if (executionStatisticsProto.hasResourceUsage()) {
                    return java.util.Optional.of<ResourceUsage?>(ResourceUsage(executionStatisticsProto.getResourceUsage()))
                } else {
                    return java.util.Optional.empty<ResourceUsage?>()
                }
            }
     ...
     * call.
     */
    class ResourceUsage(resourceUsageProto: Protos.ResourceUsage) {
        private val resourceUsageProto: Protos.ResourceUsage
        ...
        val involuntaryContextSwitches: Long
             ...
             */
            get() = resourceUsageProto.getNivcsw()
    }
}
