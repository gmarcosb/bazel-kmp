 ...
 * 
 * 
 * Instances are responsible for making a list of input files available inside the sandbox root,
 ...
 */
internal interface SandboxedSpawn : DescribableExecutionUnit {
    /** The path in which to execute the subprocess.  */
    @kotlin.jvm.JvmField
    val sandboxExecRoot: com.google.devtools.build.lib.vfs.Path?

    /** Returns `true`, if the runner should use the Subprocess timeout feature.  */
    fun useSubprocessTimeout(): Boolean {
        return false
    }

    /** Returns the path that sandbox debug output is written to, if any.  */
    val sandboxDebugPath: com.google.devtools.build.lib.vfs.Path?

    /** Returns the path where statistics about subprocess execution are written, if any.  */
    val statisticsPath: com.google.devtools.build.lib.vfs.Path?

    /**
     * Creates the sandboxed execution root, making all `inputs` available for reading, making
     * sure that the parent directories of all `outputs` and that all `writableDirs` exist
    ...
    /**
     * Copies all `outputs` to `execRoot` while keeping the directory structure.
     * 
     * 
     * An efficient implementation may move the files instead of copying them, if possible. It is
     * unspecified whether the original files still exist after this method returns.
     * 
     * @throws IOException
     */
    @Throws(IOException::class, java.lang.InterruptedException::class)
    fun copyOutputs(execRoot: com.google.devtools.build.lib.vfs.Path?)
    ...
    val interactiveDebugInstructions: java.util.Optional<String?>?
         ...
         */
        get() = java.util.Optional.empty<String?>()
}
