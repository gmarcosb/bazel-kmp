 ...
 * must be annotated with a [Command] annotation.
 */
interface BlazeCommand {
    /**
     * This method provides the imperative portion of the command. It takes a [ ] instance `options`, which provides access to the options instances
     * via [OptionsParsingResult.getOptions], and access to the residue (the remainder of
     * the command line) via [OptionsParsingResult.getResidue]. The framework parses and makes
     * available exactly the options that the command class specifies via the annotation [ ][Command.options]. The command indicates success / failure via its return value, which becomes
     ...
     * invoked from by returning the appropriate [BlazeCommandResult].
     * 
     ...
     */
    fun editOptions(optionsParser: com.google.devtools.common.options.OptionsParser?) {}
}
