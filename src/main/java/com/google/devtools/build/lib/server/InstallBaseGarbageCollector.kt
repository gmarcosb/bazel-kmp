 ...
 * 
 * 
 * Garbage collection operates on other install bases found in the parent directory of our own
 ...
 * while in use.
 */
class InstallBaseGarbageCollector internal constructor(
    root: com.google.devtools.build.lib.vfs.Path,
    ownInstallBase: com.google.devtools.build.lib.vfs.Path?,
    maxAge: java.time.Duration
) {
    private val root: com.google.devtools.build.lib.vfs.Path
    private val ownInstallBase: com.google.devtools.build.lib.vfs.Path?
    ...
    fun getRoot(): com.google.devtools.build.lib.vfs.Path {
        return root
    }

    @com.google.common.annotations.VisibleForTesting
    fun getOwnInstallBase(): com.google.devtools.build.lib.vfs.Path? {
    ...
    fun run() {
        for (dirent in root.readdir(Symlinks.FOLLOW)) {
            ...
            val child: com.google.devtools.build.lib.vfs.Path = root.getChild(dirent.getName())
            if (isInstallBase(child)) {
                if (child == ownInstallBase) {
    ...
    private fun collectWhenStale(installBase: com.google.devtools.build.lib.vfs.Path) {
        var pathToDelete: com.google.devtools.build.lib.vfs.Path? = null
        val lockPath: com.google.devtools.build.lib.vfs.Path = getLockPath(installBase)
        try {
            FileSystemLock.tryGet(lockPath, LockMode.EXCLUSIVE).use { lock ->
                val status: FileStatus? = installBase.statIfFound()
                if (status == null) {
                ...
                val age: java.time.Duration =
                    java.time.Duration.between(Instant.ofEpochMilli(status.getLastModifiedTime()), Instant.now())
    ...
    companion object {
        @com.google.common.annotations.VisibleForTesting
        const val LOCK_SUFFIX: String = ".lock"

        @com.google.common.annotations.VisibleForTesting
        const val DELETED_SUFFIX: String = ".deleted"

        private fun getLockPath(installBase: com.google.devtools.build.lib.vfs.Path): com.google.devtools.build.lib.vfs.Path {
            val parent: com.google.devtools.build.lib.vfs.Path? = installBase.getParentDirectory()
            return parent.getChild(installBase.getBaseName() + LOCK_SUFFIX)
        }

        private fun getDeletedPath(installBase: com.google.devtools.build.lib.vfs.Path): com.google.devtools.build.lib.vfs.Path {
            val parent: com.google.devtools.build.lib.vfs.Path? = installBase.getParentDirectory()
            return parent.getChild(UUID.randomUUID().toString() + DELETED_SUFFIX)
        }

        private fun isInstallBase(path: com.google.devtools.build.lib.vfs.Path): Boolean {
            val name: String = path.getBaseName()
            return name.length == 32
                    && name.chars()
                .allMatch(IntPredicate { c: Int -> (c >= 'a'.code && c <= 'f'.code) || (c >= '0'.code && c <= '9'.code) })
        }

        private fun isIncompleteDeletion(path: com.google.devtools.build.lib.vfs.Path): Boolean {
