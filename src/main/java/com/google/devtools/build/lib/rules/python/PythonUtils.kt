...
object PythonUtils {
    val INIT_PY: PathFragment? = PathFragment.create("__init__.py")
    val INIT_PYC: PathFragment? = PathFragment.create("__init__.pyc")
    val PYCACHE: PathFragment? = PathFragment.create("__pycache__")

    private val REQUIRES_INIT_PY: FileType = FileType.of(".py", ".so", ".pyc")
     ...
     */
    class GetInitPyFiles(isPackageInit: Predicate<PathFragment?>, guid: UUID) : Runfiles.EmptyFilesSupplier {
        private val isPackageInit: Predicate<PathFragment?>
         ...
         * purpose of fingerprinting this [Runfiles.EmptyFilesSupplier] instance
         */
        init {
        ...
        public override fun getExtraPaths(manifestPaths: MutableSet<PathFragment>): MutableSet<PathFragment?> {
            return getInitPyFiles(manifestPaths)
        }

        public override fun fingerprint(fp: Fingerprint) {
            fp.addUUID(guid)
        }

        /**
         * Returns the set of empty __init__.py(c) files to be added to a given set of files to allow
         * the Python runtime to find the `.py` and `.so` files present in the
         * tree.
         */
        private fun getInitPyFiles(manifestFiles: MutableSet<PathFragment>): ImmutableSet<PathFragment?> {
            val result: MutableSet<PathFragment?> = HashSet<PathFragment?>()
            // A set of directories that already have package init files.
            val hasPackageInitDirs: MutableSet<PathFragment?> = HashSet<PathFragment?>() // For b/142135992.
            ...
            for (source in manifestFiles) {
                ...
                var source: PathFragment = source
                if (REQUIRES_INIT_PY.matches(source)) {
                    ...
                    while (source.isMultiSegment()) {
                        ...
                        if (!source.endsWith(PYCACHE) && !hasPackageInitDirs.contains(source)) {
                            val initpy: PathFragment? = source.getRelative(INIT_PY)
                            val initpyc: PathFragment? = source.getRelative(INIT_PYC)
            ...
            return ImmutableSet.copyOf<PathFragment?>(result)
        }
