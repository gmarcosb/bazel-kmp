...
class SkymeldModule : BlazeModule() {
    ...
    fun determineIfMergingAnalysisExecution(env: CommandEnvironment): Boolean {
        val commandName: String = env.getCommandName()
        val packageLocator: PathPackageLocator? = env.getPackageLocator()
        val buildRequestOptions: BuildRequestOptions? =
            env.getOptions().getOptions(BuildRequestOptions::class.java)
        var effectiveValue = getPlainValueFromFlag(buildRequestOptions)
        ...
        if (effectiveValue && !buildRequestOptions.performExecutionPhase) {
            ...
            if (!(commandName == "aquery" || commandName == "cquery")) {
                logger.atInfo().log(
        ...
        if (effectiveValue
            && env.getOptions().getOptions(AnalysisOptions::class.java) != null && (env.getOptions()
                .getOptions(AnalysisOptions::class.java).getCpuHeavySkyKeysThreadPoolSize()
                    <= 0)
        ) {
            logger.atInfo().log(
        ...
        if (effectiveValue
            && env.getOptions().getOptions(CoreOptions::class.java) != null && env.getOptions()
                .getOptions(CoreOptions::class.java).getCheckLicenses()
        ) {
            logger.atInfo().log(
                "--experimental_merged_skyframe_analysis_execution is incompatible with"
    ...
    companion object {
        ...
        fun getPlainValueFromFlag(buildRequestOptions: BuildRequestOptions?): Boolean {
