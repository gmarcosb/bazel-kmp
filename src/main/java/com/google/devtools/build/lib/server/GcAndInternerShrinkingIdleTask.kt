...
/** An [IdleTask] to run the garbage collector and shrink interner pools.  */
class GcAndInternerShrinkingIdleTask(private val stateKeptAfterBuild: Boolean) :
    ...
    override fun delay(): java.time.Duration? {
        ...
        return if (stateKeptAfterBuild) java.time.Duration.ofSeconds(10) else java.time.Duration.ZERO
    }

    override fun run() {
        val memBean: java.lang.management.MemoryMXBean = java.lang.management.ManagementFactory.getMemoryMXBean()
        val before: java.lang.management.MemoryUsage = memBean.getHeapMemoryUsage()
        GoogleAutoProfilerUtils.logged("Idle GC").use { p ->
            java.lang.System.gc()
        }
        ...
        if (!java.lang.Thread.interrupted() && !stateKeptAfterBuild) {
            GoogleAutoProfilerUtils.logged("Idle interner shrinking").use { p ->
                PooledInterner.shrinkAll()
            }
        }
        val after: java.lang.management.MemoryUsage = memBean.getHeapMemoryUsage()

        logger.atInfo().log(
            "[Idle GC] used: %s -> %s, committed: %s -> %s",
            com.google.devtools.build.lib.util.StringUtilities.prettyPrintBytes(before.getUsed()),
            com.google.devtools.build.lib.util.StringUtilities.prettyPrintBytes(after.getUsed()),
            com.google.devtools.build.lib.util.StringUtilities.prettyPrintBytes(before.getCommitted()),
            com.google.devtools.build.lib.util.StringUtilities.prettyPrintBytes(after.getCommitted())
        )
    }
