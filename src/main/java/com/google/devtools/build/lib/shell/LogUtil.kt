...
internal object LogUtil {
    private const val TRUNCATE_STRINGS_AT = 150
    ...
    fun toTruncatedString(bytes: ByteArray?): String {
     ...
     */
        try {
            if (bytes.size > TRUNCATE_STRINGS_AT) {
                return (String(
                    bytes,
                    0,
                    TRUNCATE_STRINGS_AT,
                    java.nio.charset.StandardCharsets.UTF_8
                ) + "[... truncated. original size was " + bytes.size + " bytes.]")
            }
            return String(bytes)
       ...
       */
            return "IOUtil.toTruncatedString: " + e.message
        }
    }
}
