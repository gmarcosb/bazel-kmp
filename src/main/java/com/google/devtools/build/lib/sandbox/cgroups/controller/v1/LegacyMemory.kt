...
class LegacyMemory(private val path: Path) : LegacyController(), Controller.Memory {
    ...
    override fun child(name: String?): Controller.Memory {
        return LegacyMemory(getChild(name))
    }

    @Throws(IOException::class)
    override fun setMaxBytes(bytes: Long) {
        Files.writeString(path.resolve("memory.limit_in_bytes"), bytes.toString())
    }

    @Throws(IOException::class)
    override fun getMaxBytes(): Long {
        return Files.readString(path.resolve("memory.limit_in_bytes")).trim { it <= ' ' }.toLong()
    }

    @Throws(IOException::class)
    override fun getUsageInBytes(): Long {
        return Files.readString(path.resolve("memory.usage_in_bytes")).trim { it <= ' ' }.toLong()
    }
}
