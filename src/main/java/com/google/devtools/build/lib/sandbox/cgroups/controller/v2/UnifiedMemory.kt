...
class UnifiedMemory(private val path: Path) : UnifiedController(), Controller.Memory {
    override fun getPath(): Path {
        return path
    }

    @Throws(IOException::class)
    override fun child(name: String?): Controller.Memory {
        return UnifiedMemory(getChild(name))
    }

    @Throws(IOException::class)
    override fun setMaxBytes(bytes: Long) {
        Files.writeString(path.resolve("memory.max"), bytes.toString())
        // Set swap to 0 so that it doesn't unexpecedly consume more than
        // the memory limit when swap is enabled.
        Files.writeString(path.resolve("memory.swap.max"), 0L.toString())
    }

    @Throws(IOException::class)
    override fun getMaxBytes(): Long {
        return Files.readString(path.resolve("memory.max")).trim { it <= ' ' }.toLong()
    }

    @Throws(IOException::class)
    override fun getUsageInBytes(): Long {
        return Files.readString(path.resolve("memory.current")).trim { it <= ' ' }.toLong()
    }
}
