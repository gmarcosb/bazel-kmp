...
/**
 * Core options affecting a [BuildConfigurationValue] that don't belong in domain-specific
 * [FragmentOptions]. All options defined here should be universal in that they affect
 ...
 * 
 * 
 * ALSO IMPORTANT: all option types MUST define a toString method that gives identical results
 * for semantically identical option values. The simplest way to ensure that is to return the input
 * string.
 */
@OptionsClass
abstract class CoreOptions : FragmentOptions(), Cloneable {
    @Option(
        ...
        help = ("If true, runfiles of targets listed in the srcs attribute are available to targets"
                + " that consume the filegroup as a data dependency.")
    )
    abstract fun getFilegroupRunfilesForData(): Boolean
    ...
    @Option(
        ...
        help = ("Name of the scl config defined in PROJECT.scl. Note that this feature is still under"
                + " development b/324119879.")
    )
    abstract fun getSclConfig(): String?
    ...
    abstract fun getMergeGenfilesDirectory(): Boolean
    ...
    abstract fun getStarlarkExecConfig(): String?

    abstract fun setStarlarkExecConfig(value: String?)
    ...
    abstract fun getDisabledSelectOptions(): com.google.common.collect.ImmutableList<String?>?

    abstract fun setDisabledSelectOptions(value: com.google.common.collect.ImmutableList<String?>?)

    @Option(
        name = "experimental_propagate_custom_flag",
        defaultValue = "null",
        allowMultiple = true,
        converter = CustomFlagConverter::class,
        documentationCategory = OptionDocumentationCategory.UNDOCUMENTED,
        effectTags = [OptionEffectTag.AFFECTS_OUTPUTS],
        metadataTags = [OptionMetadataTag.EXPERIMENTAL],
        help = """
    ...
    abstract fun getCustomFlagsToPropagate(): MutableList<String?>?
    ...
    abstract fun getExcludeDefinesFromExecConfig(): Boolean
    ...
    @Option(
        ...
        effectTags = [OptionEffectTag.AFFECTS_OUTPUTS],
        metadataTags = [OptionMetadataTag.INCOMPATIBLE_CHANGE],
        help = ("If true, don't propagate starlark flags to the exec transition at default; only"
                + " propagate starlark flags specified in `--experimental_propagate_custom_flag`.")
    )
    abstract fun getExcludeStarlarkFlagsFromExecConfig(): Boolean
    ...
    abstract fun getPlatformInOutputDir(): TriState?
    ...
    abstract fun getUsePlatformsInOutputDirLegacyHeuristic(): Boolean
    ...
    abstract fun getOverridePlatformCpuName(): MutableList<MutableMap.MutableEntry<Label?, String?>?>?

    abstract fun setOverridePlatformCpuName(value: MutableList<MutableMap.MutableEntry<Label?, String?>?>?)
    ...
    @Option(
        ...
        effectTags = [OptionEffectTag.AFFECTS_OUTPUTS],
        metadataTags = [OptionMetadataTag.INCOMPATIBLE_CHANGE],
        help = ("Added for gradual rollout of --experimental_platform_in_output_dir to non-exec"
    ...
    abstract fun getLimitOutputDirToPlatforms(): MutableList<Label?>?
    ...
    abstract fun getIncompatibleTargetCpuFromPlatform(): Boolean

    abstract fun setIncompatibleTargetCpuFromPlatform(value: Boolean)
    ...
    @Option(
        ...
        effectTags = [OptionEffectTag.AFFECTS_OUTPUTS],
        metadataTags = [OptionMetadataTag.INCOMPATIBLE_CHANGE],
        help = ("If specified, the value of the cpu constraint (@platforms//cpu:cpu) of"
                + " the target platform is used to set the Configuration.getCpu() and"
                + " Configuration.platform_name fields in the BEP.")
    )
    abstract fun getIncompatibleBepCpuFromPlatform(): Boolean
    ...
    abstract fun getCommandLineBuildVariables(): MutableList<MutableMap.MutableEntry<String?, String?>?>?

    abstract fun setCommandLineBuildVariables(value: MutableList<MutableMap.MutableEntry<String?, String?>?>?)
    ...
    abstract fun getAllowedCpuValues(): com.google.common.collect.ImmutableList<String?>?

    abstract fun setAllowedCpuValues(value: com.google.common.collect.ImmutableList<String?>?)
    ...
    abstract fun setCpu(value: String?)

    @Option(
        name = "min_param_file_size",
        defaultValue = "32768",
        documentationCategory = OptionDocumentationCategory.UNDOCUMENTED,
        effectTags = [OptionEffectTag.LOADING_AND_ANALYSIS, OptionEffectTag.EXECUTION, OptionEffectTag.ACTION_COMMAND_LINES
        ],
        help = "Minimum command line length before creating a parameter file."
    )
    abstract fun getMinParamFileSize(): Int
    ...
    abstract fun getExtendedSanityChecks(): Boolean
    ...
    abstract fun getExperimentalWritableOutputs(): Boolean
    ...
    abstract fun getStampBinaries(): Boolean

    abstract fun setStampBinaries(value: Boolean)
    ...
    abstract fun getInstrumentationFilter(): RegexFilter?

    abstract fun setInstrumentationFilter(value: RegexFilter?)
    ...
    abstract fun getInstrumentTestTargets(): Boolean
    ...
    abstract fun getHostCpu(): String?

    @Option(
        name = "compilation_mode",
        abbrev = 'c',
        converter = com.google.devtools.build.lib.analysis.config.CompilationMode.Converter::class,
    ...
    abstract fun getCompilationMode(): CompilationMode?
    ...
    abstract fun getHostCompilationMode(): CompilationMode?
    ...
    abstract fun getUseAutoExecGroups(): Boolean
   ...
   * is being used as a configuration distinguisher for the exec transition. */
    ...
    abstract fun getPlatformSuffix(): String?

    abstract fun setPlatformSuffix(value: String?)
    ...
    abstract fun getActionEnvironment(): MutableList<EnvVar?>?

    abstract fun setActionEnvironment(value: MutableList<EnvVar?>?)
    ...
    abstract fun getHostActionEnvironment(): MutableList<EnvVar?>?

    abstract fun setHostActionEnvironment(value: MutableList<EnvVar?>?)
    ...
    abstract fun getCollectCodeCoverage(): Boolean
    ...
    @Option(
        ...
        metadataTags = [OptionMetadataTag.EXPERIMENTAL],
        effectTags = [OptionEffectTag.AFFECTS_OUTPUTS],
        help = ("If specified, Bazel will also generate collect coverage information for generated"
                + " files.")
    )
    abstract fun getCollectCodeCoverageForGeneratedFiles(): Boolean
    ...
    @Option(
        ...
        documentationCategory = OptionDocumentationCategory.OUTPUT_SELECTION,
        effectTags = [OptionEffectTag.AFFECTS_OUTPUTS],
        help = ("If true, write runfiles manifests for all targets. If false, omit them. Local tests will"
                + " fail to run when false.")
    )
    abstract fun getBuildRunfileManifests(): Boolean
    ...
    abstract fun getBuildRunfileLinks(): Boolean
    ...
    abstract fun getAlwaysIncludeFilesToBuildInData(): Boolean
    ...
    abstract fun getCompactRepoMapping(): Boolean
    ...
    abstract fun getRunUnder(): RunUnder?

    abstract fun setRunUnder(value: RunUnder?)
    ...
    abstract fun getCheckVisibility(): Boolean

    abstract fun setCheckVisibility(value: Boolean)
    ...
    @Option(
        ...
        effectTags = [OptionEffectTag.BUILD_FILE_SEMANTICS],
        metadataTags = [OptionMetadataTag.EXPERIMENTAL],
        help = ("If true, enable package()s to set the transitive_visibility attribute to restrict which"
                + " packages may depend on them.")
    )
    abstract fun getEnforceTransitiveVisibility(): Boolean
    ...
    abstract fun getVerboseVisibilityErrors(): Boolean
    ...
    @Option(
        ...
        effectTags = [OptionEffectTag.BUILD_FILE_SEMANTICS],
        metadataTags = [OptionMetadataTag.INCOMPATIBLE_CHANGE],
        help = ("If enabled, check testonly for prerequisite targets that are output files by"
                + " looking up the testonly of the generating rule. This matches visibility"
                + " checking.")
    )
    abstract fun getCheckTestonlyForOutputFiles(): Boolean
    ...
    abstract fun getCheckLicenses(): Boolean
    ...
    abstract fun getEnforceConstraints(): Boolean
    ...
    abstract fun getActionListeners(): MutableList<Label?>?
    ...
    abstract fun getIsExec(): Boolean
    ...
    abstract fun getAllowAnalysisFailures(): Boolean
    ...
    abstract fun getEvaluatingForAnalysisTest(): Boolean

    abstract fun setEvaluatingForAnalysisTest(value: Boolean)
    ...
    abstract fun getAnalysisTestingDepsLimit(): Int
    ...
    abstract fun getDefaultFeatures(): MutableList<String>?

    abstract fun setDefaultFeatures(value: MutableList<String?>?)
    ...
    abstract fun getHostFeatures(): MutableList<String?>?

    abstract fun setHostFeatures(value: MutableList<String?>?)
    ...
    abstract fun getTargetEnvironments(): MutableList<Label?>?
    ...
    abstract fun getAllowUnresolvedSymlinks(): Boolean
    ...
    @Option(
        ...
        help = ("If enabled, Bazel allows the use of ctx.actions.map_directory(). This allows for the"
    ...
    abstract fun getAllowMapDirectory(): Boolean

    /** Values for --experimental_output_paths.  */
    enum class OutputPathsMode {
        /** Use the production output path model.  */
        OFF,

        /**
         * Strip the config prefix (i.e. `/x86-fastbuild/` from output paths for actions that are
         * registered to support this feature.
         * 
         * 
         * See [com.google.devtools.build.lib.actions.PathStripper] for details.
         */
        STRIP,
    }

    /** Converter for --experimental_output_paths.  */
    class OutputPathsConverter : EnumConverter<OutputPathsMode?>(OutputPathsMode::class.java, "output path mode")
    ...
    abstract fun getOutputPathsMode(): OutputPathsMode?
    ...
    abstract fun getEnableRunfiles(): TriState?

    @Option(
        name = "modify_execution_info",
        allowMultiple = true,
        converter = com.google.devtools.build.lib.analysis.config.ExecutionInfoModifier.Converter::class,
        defaultValue = "null",
    ...
    abstract fun getExecutionInfoModifier(): MutableList<ExecutionInfoModifier?>?

    abstract fun setExecutionInfoModifier(value: MutableList<ExecutionInfoModifier?>?)
    ...
    abstract fun getAdditiveModifyExecutionInfo(): Boolean
    ...
    abstract fun getBazelTestExecRunUnder(): Boolean
    ...
    abstract fun getIncludeRequiredConfigFragmentsProvider(): IncludeConfigFragmentsEnum?
    ...
    abstract fun getRemotableSourceManifestActions(): Boolean

    @Option(
        name = "flag_alias",
        converter = FlagAliasConverter::class,
        defaultValue = "null",
        allowMultiple = true,
    ...
    abstract fun getCommandLineFlagAliases(): MutableList<MutableMap.MutableEntry<String?, Label?>?>?

    abstract fun setCommandLineFlagAliases(value: MutableList<MutableMap.MutableEntry<String?, Label?>?>?)
    ...
    @Option(
        ...
        effectTags = [OptionEffectTag.LOADING_AND_ANALYSIS, OptionEffectTag.EXECUTION],
        converter = RegexFilter.RegexFilterConverter::class,
        help = ("Regex filter for mnemonics of actions for which we should create archived tree"
                + " artifacts. This option is a no-op for actions which do not generate tree"
                + " artifacts.")
    )
    abstract fun getArchivedArtifactsMnemonicsFilter(): RegexFilter?

    abstract fun setArchivedArtifactsMnemonicsFilter(value: RegexFilter?)
    ...
    abstract fun getDebugSelectsAlwaysSucceed(): Boolean
    ...
    abstract fun getThrottleActionCacheCheck(): Boolean
    ...
    @Option(
        ...
        deprecationWarning = ("Tests select an execution platform matching all constraints of the target platform by"
    ...
    abstract fun getUseTargetPlatformForTests(): Boolean
    ...
    @Option(
        ...
        effectTags = [OptionEffectTag.LOADING_AND_ANALYSIS],
        allowMultiple = true,
        help = ("Comma-separated list of aspects to be applied to exec-configured targets, regardless of"
                + " whether or not they are top-level targets. This is an experimental feature and"
                + " is subject to change.")
    )
    abstract fun getExecAspects(): MutableList<String?>?

    fun getPlatformCpuNameOverride(platform: Label?): java.util.Optional<String?> {
        // As highest priority, use the last entry that matches in name override option.
        return com.google.common.collect.Streams.findLast<String?>(
            getOverridePlatformCpuName().stream()
                .filter(java.util.function.Predicate { e: MutableMap.MutableEntry<Label?, String?>? ->
                    e.getKey().equals(platform)
                })
                .map<String?>(java.util.function.Function { java.util.Map.Entry.getValue() })
        )
    }

    fun usePlatformInOutputDir(platform: Label?): Boolean {
        if (getIsExec()) {
    ...
    fun getCommandLineFlagAliasesMap(): com.google.common.collect.ImmutableMap<String?, Label?>? {
        return ALIAS_MAP_CACHE.get(getCommandLineFlagAliases())
    }

    /** Ways configured targets may provide the [Fragment]s they require.  */
    enum class IncludeConfigFragmentsEnum {
        /**
    ...
    class IncludeConfigFragmentsEnumConverter

        : EnumConverter<IncludeConfigFragmentsEnum?>(
        IncludeConfigFragmentsEnum::class.java,
        "include config fragments provider option"
    )

    /** Normalizes --define flags, preserving the last one to appear in the event of conflicts. */
    fun getNormalizedCommandLineBuildVariables(): com.google.common.collect.ImmutableMap<String?, String?> {
        return Companion.sortEntries<V?>(normalizeEntries(getCommandLineBuildVariables())).stream()
            .collect(
    ...
    public override fun getNormalized(): CoreOptions {
        val result = clone() as CoreOptions
        result.setAllowedCpuValues(dedupAndSort(getAllowedCpuValues()))
        result.setCommandLineBuildVariables(
            Companion.sortEntries<V?>(normalizeEntries(getCommandLineBuildVariables()))
        )

        // Normalize features.
        result.setDefaultFeatures(Companion.getNormalizedFeatures(getDefaultFeatures()!!))
        ...
        result.setCommandLineFlagAliases(Companion.sortEntries<V?>(normalizeEntries(getCommandLineFlagAliases())))

        return result
    }

    public override fun clone(): CoreOptions? {
        return super.clone() as CoreOptions?
    }

    companion object {
        private val ALIAS_MAP_CACHE: com.github.benmanes.caffeine.cache.LoadingCache<MutableList<MutableMap.MutableEntry<String?, Label?>?>?, com.google.common.collect.ImmutableMap<String?, Label?>?> =
            Caffeine.newBuilder().weakKeys()
                .build<MutableList<MutableMap.MutableEntry<String?, Label?>?>?, com.google.common.collect.ImmutableMap<String?, Label?>?>(
                    com.github.benmanes.caffeine.cache.CacheLoader { entries: Iterable<out MutableMap.MutableEntry<out K?, out V?>?>? ->
        ...
        private fun <V> sortEntries(entries: MutableList<MutableMap.MutableEntry<String?, V?>?>): MutableList<MutableMap.MutableEntry<String?, V?>?> {
            val sortedEntries: com.google.common.collect.ImmutableList<MutableMap.MutableEntry<String?, V?>?> =
                entries.stream().sorted(java.util.Map.Entry.comparingByKey<String?, V?>())
        ...
        private fun getNormalizedFeatures(features: MutableList<String>): MutableList<String?> {
            ...
            val enabled: TreeSet<String?> = TreeSet<String?>()
            val disabled: TreeSet<String?> = TreeSet<String?>()
            for (entry in featureToState.entrySet()) {
                if (entry.getValue()) {
            ...
            val result: MutableList<String?> = java.util.ArrayList<String?>(enabled)
            disabled.stream().map<String?>(java.util.function.Function { x: String? -> "-" + x })
                .forEach(java.util.function.Consumer { e: String? -> result.add(e) })
            // If we made no changes, return the same instance we got to reduce churn.
            return if (result == features) features else result
        }
    }
}
