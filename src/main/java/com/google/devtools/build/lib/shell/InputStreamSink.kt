 ...
 * an [OutputStream], or discarded.
 * 
 * 
 * This class is useful for handing the `stdout` and `stderr`
 * streams from a [Process] started with [Runtime.exec].
 * If these streams are not consumed, the Process may block resulting in a
 * deadlock.
 * 
 * @see [
 * JavaWorld: When Runtime.exec
](http://www.javaworld.com/javaworld/jw-12-2000/jw-1229-traps.html) */
object InputStreamSink {
    /**
     * Black hole into which bytes are sometimes discarded by [NullSink].
     * It is shared by all threads since the actual contents of the buffer
     * are irrelevant.
     */
    private val DISCARD = ByteArray(4096)

    /**
     * Creates a [Runnable] which consumes the provided
     * [InputStream] 'in', discarding its contents.
     */
    fun newRunnableSink(`in`: java.io.InputStream): java.lang.Runnable {
        if (`in` == null) {
            throw java.lang.NullPointerException("in")
        }
        return NullSink(`in`)
    }

    /**
     * Creates a [Runnable] which copies everything from 'in'
     * to 'out'. 'out' will be written to and flushed after each
     * read from 'in'. However, 'out' will not be closed.
     */
    fun newRunnableSink(`in`: java.io.InputStream, out: java.io.OutputStream): java.lang.Runnable {
        if (`in` == null) {
            throw java.lang.NullPointerException("in")
        }
        if (out == null) {
            throw java.lang.NullPointerException("out")
        }
        return CopySink(`in`, out)
    }

    /**
     * A [Thread] which reads and discards data from an
     * [InputStream].
     */
    private class NullSink(`in`: java.io.InputStream) : java.lang.Runnable {
        ...
        override fun run() {
            try {
                try {
                    ...
                    do {
                        `in`.skip(Int.Companion.MAX_VALUE.toLong())
    ...
    /**
     * A [Thread] which reads data from an [InputStream],
     * and translates it into an [OutputStream].
     */
    private class CopySink(`in`: java.io.InputStream, out: java.io.OutputStream) : java.lang.Runnable {
        private val `in`: java.io.InputStream
