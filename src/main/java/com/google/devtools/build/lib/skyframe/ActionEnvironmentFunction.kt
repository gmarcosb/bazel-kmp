 ...
 */
class ActionEnvironmentFunction : SkyFunction {
    @Throws(java.lang.InterruptedException::class)
    override fun compute(skyKey: SkyKey, env: SkyFunction.Environment): SkyValue? {
        val actionEnv: MutableMap<String?, String?>? = PrecomputedValue.ACTION_ENV.get(env)
        val key = skyKey.argument() as String?
        if (actionEnv!!.containsKey(key) && actionEnv.get(key) != null) {
            return EnvironmentVariableValue(actionEnv.get(key))
        }
    ...
    internal class Key private constructor(arg: String?) : AbstractSkyKey<String?>(arg) {
        ...
        val skyKeyInterner: SkyKeyInterner<Key?>
            get() = com.google.devtools.build.lib.skyframe.ActionEnvironmentFunction.Key.Companion.interner

        companion object {
            private val interner: SkyKeyInterner<Key?> = SkyKey.newInterner<Key?>()

            private fun create(arg: String?): Key {
                return com.google.devtools.build.lib.skyframe.ActionEnvironmentFunction.Key.Companion.interner.intern(
                    com.google.devtools.build.lib.skyframe.ActionEnvironmentFunction.Key(arg)
                )
            }
    ...
    companion object {
        ...
        fun key(variable: String?): Key {
            return com.google.devtools.build.lib.skyframe.ActionEnvironmentFunction.Key.Companion.create(variable)
        }
        ...
        ): com.google.common.collect.ImmutableMap<String?, java.util.Optional<String?>?>? {
            val skyframeKeys: com.google.common.collect.ImmutableSet<Key?> =
                keys.stream().map<Key?> { variable: String? -> key(variable) }
                    .collect(com.google.common.collect.ImmutableSet.toImmutableSet<Key?>())
            val values: SkyframeLookupResult = env.getValuesAndExceptions(skyframeKeys)
            if (env.valuesMissing()) {
                return null
            }

            val result: com.google.common.collect.ImmutableMap.Builder<String?, java.util.Optional<String?>?> =
                com.google.common.collect.ImmutableMap.builderWithExpectedSize<String?, java.util.Optional<String?>?>(
                    skyframeKeys.size
                )
            for (key in skyframeKeys) {
