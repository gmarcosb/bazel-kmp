...
internal abstract class UnifiedController : Controller {
    override fun isLegacy(): Boolean {
        return false
    }

    @Throws(IOException::class)
    protected fun getChild(name: String?): Path {
        val subtree = getPath().resolve("cgroup.subtree_control").toFile()
        val controllers = getPath().resolve("cgroup.controllers").toFile()
        if (subtree.canWrite() && controllers.canRead()) {
            val sink = Files.asCharSink(subtree, StandardCharsets.UTF_8)
            Scanner(controllers, StandardCharsets.UTF_8).use { scanner ->
                sink.writeLines(Streams.stream<String?>(scanner).map<String?> { c: String? -> "+" + c }, " ")
            }
        }
        val path = getPath().resolve(name)
        path.toFile().mkdirs()
        return path
    }
}
