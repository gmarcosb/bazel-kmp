...
interface TestingModuleApi : StarlarkValue {
    @StarlarkMethod(
        name = "ExecutionInfo", doc = ("<a href='../providers/ExecutionInfo.html'>testing.ExecutionInfo</a> provider"
                + " key/constructor"), structField = true
    )
    @Throws(EvalException::class)
    fun executionInfo(): ExecutionInfoApiProvider?

    @StarlarkMethod(
        name = "TestEnvironment",
        doc = ("<b>Deprecated: Use RunEnvironmentInfo instead.</b> Creates a new test environment "
        ...
        parameters = [Param(
            ...
            named = true,
            positional = true,
            doc = ("A sequence of names of environment variables. These variables are made available"
    ...
    @Throws(EvalException::class)
    fun testEnvironment(
        environment: Dict<*, *>?,  // <String, String> expected
        inheritedEnvironment: Sequence<*>? /* <String> expected */
    ): RunEnvironmentInfoApi?

    @StarlarkMethod(
        name = "analysis_test",
        doc = ("Creates a new analysis test target. <p>The number of transitive dependencies of the test"
                + " are limited. The limit is controlled by"
                + " <code>--analysis_testing_deps_limit</code> flag."),
        parameters = [Param(
            name = "name",
            named = true,
            doc = "Name of the target. It should be a Starlark identifier, matching pattern"
                    + " '[A-Za-z_][A-Za-z0-9_]*'."
        ), Param(
            name = "implementation",
            named = true,
            doc = ("The Starlark function implementing this analysis test. It must have exactly one"
            ...
            named = true,
            defaultValue = "{}",
            doc = ("Dictionary declaring the attributes. See the <a"
            ...
            named = true,
            defaultValue = "[]",
            doc = "List of configuration fragments that are available to the implementation of the"
                    + " analysis test."
        ), Param(
            name = StarlarkRuleFunctionsApi.Companion.TOOLCHAINS_PARAM,
            allowedTypes = [ParamType(type = Sequence::class, generic1 = Any::class)],
            named = true,
            defaultValue = "[]",
            doc = "The set of toolchains the test requires. See the <a"
