...
class Hierarchy(id: Int, controllers: com.google.common.collect.ImmutableList<String?>?, path: java.nio.file.Path?) {
    val isV2: Boolean
        get() = this.controllers.size == 1 && this.controllers.contains("") && this.id == 0

    val id: Int
    val controllers: com.google.common.collect.ImmutableList<String?>?
    ...
    init {
        ...
        this.id = id
        java.util.Objects.requireNonNull<com.google.common.collect.ImmutableList<String?>?>(controllers, "controllers")
        java.util.Objects.requireNonNull<java.nio.file.Path?>(path, "path")
    }

    companion object {
        /**
         * A regexp that matches entries in `/proc/self/cgroup`.
         * 
         * 
         * The format is documented in https://man7.org/linux/man-pages/man7/cgroups.7.html
         */
        private val PROC_CGROUPS_PATTERN: java.util.regex.Pattern =
            java.util.regex.Pattern.compile("^(?<id>\\d+):(?<controllers>[^:]*):(?<file>.+)")

        fun create(
            id: Int,
            controllers: com.google.common.collect.ImmutableList<String?>?,
            path: java.nio.file.Path?
        ): Hierarchy {
            return com.google.devtools.build.lib.sandbox.cgroups.Hierarchy(id, controllers, path)
        }

        @Throws(IOException::class)
        fun parse(procCgroup: java.io.File): com.google.common.collect.ImmutableList<Hierarchy?> {
            val hierarchies: com.google.common.collect.ImmutableList.Builder<Hierarchy?> =
                com.google.common.collect.ImmutableList.builder<Hierarchy?>()
            for (line in com.google.common.io.Files.readLines(procCgroup, java.nio.charset.StandardCharsets.UTF_8)) {
                val m: java.util.regex.Matcher =
                    com.google.devtools.build.lib.sandbox.cgroups.Hierarchy.Companion.PROC_CGROUPS_PATTERN.matcher(line)
                if (!m.matches()) {
                    continue
                }

                val id: Int = m.group("id").toInt()
                val path: String? = m.group("file")
                val controllers: com.google.common.collect.ImmutableList<String?> =
                    com.google.common.collect.ImmutableList.copyOf<String?>(
                        m.group("controllers").split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                    )
                hierarchies.add(
                    com.google.devtools.build.lib.sandbox.cgroups.Hierarchy.Companion.create(
                        id,
                        controllers,
                        Paths.get(path)
                    )
