...
/** A factory for creating [VirtualCgroup] instances.  */
class VirtualCgroupFactory(
    name: String?,
    root: VirtualCgroup?,
    defaultLimits: com.google.common.collect.ImmutableMap<String?, Double?>?,
    private val alwaysCreate: Boolean
) {
    private val name: String
    private val defaultLimits: com.google.common.collect.ImmutableMap<String?, Double?>
    private val root: VirtualCgroup
    private val cgroups: ConcurrentHashMap<Int?, VirtualCgroup?>

    init {
        this.name = com.google.common.base.Preconditions.checkNotNull<String>(name)
        this.defaultLimits =
            com.google.common.base.Preconditions.checkNotNull<com.google.common.collect.ImmutableMap<String?, Double?>>(
                defaultLimits
            )
        this.root = com.google.common.base.Preconditions.checkNotNull<VirtualCgroup>(root)
        this.cgroups = ConcurrentHashMap<Int?, VirtualCgroup?>()
    }
    ...
    fun create(id: Int?, limits: com.google.common.collect.ImmutableMap<String?, Double?>): VirtualCgroup? {
        if (!alwaysCreate && defaultLimits.isEmpty() && limits.isEmpty()) {
            return VirtualCgroup.Companion.NULL
        }

        val cpuLimit: Double? = limits.getOrDefault("cpu", defaultLimits.getOrDefault("cpu", 0.0))
        val memoryLimit: Long = ((limits.getOrDefault("memory", defaultLimits.getOrDefault("memory", 0.0))
                * 1024
                * 1024)).toLong()

        if (!alwaysCreate && cpuLimit == 0.0 && memoryLimit == 0L) {
            return VirtualCgroup.Companion.NULL
        }

        val childName = this.name + id + ".scope"
        val cgroup: VirtualCgroup = root.createChild(childName)
        ...
        if (cpuLimit!! > 0 && cgroup.cpu() != null) {
            ...
            if (cgroup.cpu().getCpus().toDouble() == cpuLimit) {
    ...
    fun get(id: Int?): VirtualCgroup? {
        return cgroups.get(id)
    }

    /** Removes the cgroup with the given id.  */
    @com.google.errorprone.annotations.CanIgnoreReturnValue
    fun remove(id: Int?): VirtualCgroup? {
        val cgroup: VirtualCgroup? = cgroups.remove(id)
        if (cgroup != null) {
            cgroup.destroy()
        }
        return cgroup
    ...
    companion object {
        ...
        /** A factory that always returns [VirtualCgroup.NULL].  */
        val NOOP: VirtualCgroupFactory = VirtualCgroupFactory(
            "noop_",
            VirtualCgroup.Companion.NULL,
