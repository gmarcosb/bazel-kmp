...
/**
 * Implementation of various settings for the `--auto_output_filter`
 * option.
 * 
 * 
 * Note that any actions owned by the system owner (typically just the build info
 * action) are treated specially: They are not filtered out by these auto filters.
 */
enum class AutoOutputFilter {
    /**
     * Generates an empty output filter (i.e. one that matches everything).
     */
    NONE {
        override fun getFilter(targets: Iterable<Label?>?): OutputFilter? {
     ...
     * package as a target on the command line. `//java/foo` and
     * `//javatests/foo` are treated like a single package.
     */
    PACKAGES {
        override fun getFilter(targets: Iterable<Label>): OutputFilter? {
            val pattern = Pattern.compile(
                (AutoOutputFilter.Companion.SYSTEM_ACTION_REGEX + "|"
     ...
     * `//java/foo` and `//javatests/foo` are treated like a single
     * package.
     */
    SUBPACKAGES {
        override fun getFilter(targets: Iterable<Label>): OutputFilter? {
            val packages: MutableList<String> = ArrayList<String>()

            var previous: String? = null
            for (pkg in getPackages(targets)) {
    ...
    abstract fun getFilter(targets: Iterable<Label?>?): OutputFilter?

    /**
     * Returns the package names of a some targets as an alphabetically sorted list. If there are
     * targets under `//java/...` or `//javatests/...`, a string starting with
     * "java(tests)?" is added instead.
     */
    protected fun getPackages(targets: Iterable<Label>): SortedSet<String> {
        val packages = ImmutableSortedSet.naturalOrder<String?>()
        for (label in targets) {
            var name: String = label.getPackageName()
     ...
     */
    protected fun getPkgRegex(strings: Iterable<String>): String {
        return "(" + Joiner.on('|').join(strings) + ")"
    }

    /**
     * A converter for the `--auto_output_filter` option.
     */
    class Converter : EnumConverter<AutoOutputFilter?>(AutoOutputFilter::class.java, "automatic output filter")
    companion object {
        private val SYSTEM_ACTION_REGEX = "(unknown)|" + PlatformConstants.INTERNAL_PLATFORM.getCanonicalForm()
    }
}
