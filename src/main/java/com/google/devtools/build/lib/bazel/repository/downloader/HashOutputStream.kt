 ...
 * 
 * 
 * The actual checksum is computed gradually as the output is written. If it doesn't match, then
 * an [IOException] will be thrown when [.close] is called. This error will be thrown
 * multiple times if these methods are called again for some reason.
 * 
 * 
 * Note that as the checksum can only be computed once the stream is closed, data will be written
...
class HashOutputStream(@param:WillCloseWhenClosed private val delegate: OutputStream, private val checksum: Checksum) :
    OutputStream() {
