...
class WindowsPowershellCommandConstructor internal constructor(private val scriptNameSuffix: String) :
    CommandConstructor {
    override fun asExecArgv(command: String?): com.google.common.collect.ImmutableList<String?> {
        ...
        return com.google.common.collect.ImmutableList.of<String?>(
            "powershell.exe",
            "/c",
            POWERSHELL_SETUP_COMMANDS + command
        )
    }
    ...
    override fun commandAsScript(ruleContext: RuleContext, command: String?): Artifact? {
        ...
        return FileWriteAction.Companion.createFile(ruleContext, scriptFileName, command,  /*executable=*/true)
    }

    companion object {
        private val POWERSHELL_SETUP_COMMANDS =  // 1. Use Set-ExecutionPolicy to allow users to run scripts unsigned.
            ("Set-ExecutionPolicy -Scope CurrentUser RemoteSigned; " // 2. Set $errorActionPreference to Stop so that we exit immediately if a CmdLet
                    // fails,
                    //    but when an external command fails, it will still continue.
                    + "\$errorActionPreference='Stop'; " // 3. Change the default encoding to utf-8, by default it was utf-16.
