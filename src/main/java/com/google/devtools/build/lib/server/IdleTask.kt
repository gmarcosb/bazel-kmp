...
/** A task to be executed by [IdleTaskManager] while the server is idle.  */
interface IdleTask {
    ...
    class Result(@kotlin.jvm.JvmField val name: String?, @kotlin.jvm.JvmField val status: Status?, runningTime: java.time.Duration?) {
        val runningTime: java.time.Duration?
     ...
     * 
     * 
     * The [IdleTaskManager] will wait for at least this long, but may delay execution
     * further if many cleanup tasks were registered.
     */
    fun delay(): java.time.Duration? {
     ...
     * 
     * 
     * Implementations are encouraged to handle interruption promptly by throwing [ ], but are not required to do so, and may instead opt to complete normally.
     * Either way, the [IdleTaskManager] will wait for the task to finish before leaving the
