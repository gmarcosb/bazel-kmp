 ...
 * accumulating ([.createAccumulatingConsumers],
 * and streaming ([.createStreamingConsumers]).
 */
internal object Consumers {
    private val logger: GoogleLogger = GoogleLogger.forEnclosingClass()

    private val pool: ExecutorService = Executors.newCachedThreadPool(AccumulatorThreadFactory())

    fun createAccumulatingConsumers(): OutErrConsumers {
        return OutErrConsumers(AccumulatingConsumer(), AccumulatingConsumer())
    }

    fun createStreamingConsumers(out: java.io.OutputStream?, err: java.io.OutputStream?): OutErrConsumers {
        com.google.common.base.Preconditions.checkNotNull<java.io.OutputStream?>(out)
        com.google.common.base.Preconditions.checkNotNull<java.io.OutputStream?>(err)
        return OutErrConsumers(StreamingConsumer(out), StreamingConsumer(err))
    }

    /**
     * Close the `in` stream and log a warning if anything happens.
     */
    private fun silentClose(closeable: java.io.Closeable) {
        try {
            closeable.close()
        } catch (ioe: IOException) {
            com.google.devtools.build.lib.shell.Consumers.logger.atWarning().withCause(ioe)
                .log("Unexpected exception while closing input stream")
        }
    }

    internal class OutErrConsumers private constructor(
        private val out: OutputConsumer,
        private val err: OutputConsumer
    ) {
        fun registerInputs(outInput: java.io.InputStream?, errInput: java.io.InputStream?, closeStreams: Boolean) {
        ...
        val accumulatedOut: java.io.ByteArrayOutputStream?
            get() = out.accumulatedOut

        val accumulatedErr: java.io.ByteArrayOutputStream?
            get() = err.accumulatedOut

        fun logConsumptionStrategy() {
     ...
     * InputStream provided by [.registerInput].
    ...
    private interface OutputConsumer {
         ...
         * [CommandResult.NO_OUTPUT_COLLECTED] if it doesn't accumulate
         * any output.
         * 
        ...
        fun registerInput(`in`: java.io.InputStream?, closeConsumer: Boolean)
    ...
    private class StreamingConsumer(out: java.io.OutputStream?) : FutureConsumption() {
        ...
        override fun getAccumulatedOut(): java.io.ByteArrayOutputStream {
            return CommandResult.Companion.NO_OUTPUT_COLLECTED
        }

        override fun logConsumptionStrategy() {
            com.google.devtools.build.lib.shell.Consumers.logger.atFiner()
                .log("Output will be sent to streams provided by client")
        }

        override fun createConsumingAndClosingSink(
            `in`: java.io.InputStream,
            closeConsumer: Boolean
        ): java.lang.Runnable {
            return ClosingSink(`in`, out, closeConsumer)
        }
    }

    /**
     * This consumer sends the input to a [ByteArrayOutputStream]
     * while consuming it. This accumulated stream can be obtained by
     * calling [.getAccumulatedOut].
     */
    private class AccumulatingConsumer : FutureConsumption() {
        private val out: java.io.ByteArrayOutputStream = java.io.ByteArrayOutputStream()
        ...
        override fun logConsumptionStrategy() {
            com.google.devtools.build.lib.shell.Consumers.logger.atFiner()
                .log("Output will be accumulated (promptly read off) and returned")
        }

        public override fun createConsumingAndClosingSink(
            `in`: java.io.InputStream,
            closeConsumer: Boolean
        ): java.lang.Runnable {
            return ClosingSink(`in`, out)
        }
    }

    /**
     * A mixin that makes consumers active - this is where we kick of
     * multithreading ([.registerInput]), cancel actions
     * and wait for the consumers to complete.
     */
    private abstract class FutureConsumption : OutputConsumer {
        private var future: java.util.concurrent.Future<*>? = null

        override fun registerInput(`in`: java.io.InputStream?, closeConsumer: Boolean) {
            val sink: java.lang.Runnable? = createConsumingAndClosingSink(`in`, closeConsumer)
            future = com.google.devtools.build.lib.shell.Consumers.pool.submit(sink)
        }

        protected abstract fun createConsumingAndClosingSink(
            `in`: java.io.InputStream?,
            close: Boolean
        ): java.lang.Runnable?
        ...
        override fun waitForCompletion() {
            try {
                ...
                val nested: Throwable? = ee.cause
                if (nested is java.lang.RuntimeException) {
                    ...
                    val cause = nested.cause
                    if (cause is IOException) {
                        throw cause
                    } else {
                        throw nested
                    }
                } else if (nested is java.lang.OutOfMemoryError) {
                    // OutOfMemoryError does not support exception chaining.
                    throw nested as java.lang.OutOfMemoryError
                } else if (nested is java.lang.Error) {
                    throw java.lang.Error("unhandled Error in worker thread", ee)
                } else {
                    throw java.lang.RuntimeException("unknown execution problem", ee)
                }
            }
        }
    }

    private class AccumulatorThreadFactory : ThreadFactory {
        override fun newThread(runnable: java.lang.Runnable?): java.lang.Thread {
        ...
        companion object {
            private val threadInitNumber: AtomicInteger = AtomicInteger(0)
        }
    }
     ...
     */
    private class ClosingSink : java.lang.Runnable {
        private val `in`: java.io.InputStream
        private val out: java.io.OutputStream?
        private val sink: java.lang.Runnable
        private val close: Boolean

        /**
         * Creates a sink that will read `in` and discard it.
         */
        ...
        /**
         * Creates a sink that will pump InputStream `in`
         * into OutputStream `out`.
         */
        @kotlin.jvm.JvmOverloads
        internal constructor(`in`: java.io.InputStream, out: java.io.OutputStream?, close: Boolean = false) {
        ...
        override fun run() {
            try {
                ...
                com.google.devtools.build.lib.shell.Consumers.silentClose(`in`)
                if (close && out != null) {
                    com.google.devtools.build.lib.shell.Consumers.silentClose(out)
                }
            }
        }
    }
