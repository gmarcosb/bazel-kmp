 ...
 * 
 * 
 * We use this code in preference to using sun.misc directly since the latter is deprecated, and
 ...
 */
abstract class InterruptSignalHandler protected constructor() : Runnable {
    private var oldHandler: SignalHandler?
     ...
     */
    init {
        this.oldHandler = Signal.handle(SIGINT, SignalHandler { signal: Signal? -> run() })
    }

    /** Disables SIGINT handling.  */
    @kotlin.jvm.Synchronized
    fun uninstall() {
        Preconditions.checkNotNull<SignalHandler?>(oldHandler, "uninstall() already called")
        Signal.handle(SIGINT, oldHandler)
        oldHandler = null
    }

    companion object {
        private val SIGINT = Signal("INT")
    }
}
