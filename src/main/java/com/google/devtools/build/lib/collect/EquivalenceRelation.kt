 ...
 * 
 * 
 * The ordering imposed by an EquivalenceRelation <tt>e</tt> on a set of elements <tt>S</tt> is
 * said to be *consistent with equals* if and only if <tt>(compare((Object)e1,
 ...
 * 
 * 
 * Unlike [java.util.Comparator], whose implementations are often consistent with equals,
 ...
 * 
 * 
 * For example, consider possible equivalence relations for [java.lang.Integer]: the
 * default equivalence defined by Integer.equals() is based on the integer value is represents, but
 * two alternative equivalences would be [EquivalenceRelation.IDENTITY] (object
 * identitya more discriminative relation) or *parity* (under which all even numbers,
 * odd numbers are considered equivalent to each othera less discriminative relation).
 */
// TODO: Consider phasing out this interface in favour of com.google.common.base.Equivalence
fun interface EquivalenceRelation<T> {
    // This should be a superinterface of Comparator.
    /**
     * Compares its two arguments for equivalence.  Returns zero if they are
     * considered equivalent, or non-zero otherwise.
     *
     *
     * 
    ...
    companion object {
         ...
         * instance.
         */
        val IDENTITY: EquivalenceRelation<*> = EquivalenceRelation { o1: Any?, o2: Any? -> if (o1 === o2) 0 else -1 }
         ...
         */
        val DEFAULT: EquivalenceRelation<*> =
            EquivalenceRelation { o1: Any?, o2: Any? -> if (if (o1 == null) o2 == null else (o1 == o2)) 0 else -1 }
    }
}
