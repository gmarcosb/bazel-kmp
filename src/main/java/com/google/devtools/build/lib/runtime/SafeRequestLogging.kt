...
object SafeRequestLogging {
    private val suppressFromLog: java.util.regex.Pattern = java.util.regex.Pattern.compile(
        "--client_env=([^=]*(?:auth|pass|cookie|token|api_key)[^=]*)=", java.util.regex.Pattern.CASE_INSENSITIVE
    )
     ...
     */
    fun getRequestLogString(requestStrings: MutableList<String?>): String {
        val buf: java.lang.StringBuilder = java.lang.StringBuilder()
        buf.append('[')
        var sep = ""
        val m: java.util.regex.Matcher = suppressFromLog.matcher("")
        for (s in requestStrings) {
            buf.append(sep)
            m.reset(s)
            if (m.lookingAt()) {
                buf.append(m.group())
