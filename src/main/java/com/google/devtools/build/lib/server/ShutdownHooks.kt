...
class ShutdownHooks private constructor() {
    private val enabled: AtomicBoolean = AtomicBoolean(true)

    @javax.annotation.concurrent.GuardedBy("this")
    private val filesToDeleteAtExit: MutableList<com.google.devtools.build.lib.vfs.Path?> =
        java.util.ArrayList<com.google.devtools.build.lib.vfs.Path?>()

    @javax.annotation.concurrent.GuardedBy("this")
    private var pidFileCleanup: java.lang.Runnable? = null

    /** Schedules the specified file for (attempted) deletion at JVM exit.  */
    @kotlin.jvm.Synchronized
    fun deleteAtExit(path: com.google.devtools.build.lib.vfs.Path?) {
     ...
     * 
     * 
     * The PID file watcher will be stopped before the PID file is deleted.
     * 
     * 
     * This operation will be executed when the JVM shuts down.
     */
    @kotlin.jvm.Synchronized
    fun cleanupPidFile(pidFile: com.google.devtools.build.lib.vfs.Path, pidFileWatcher: PidFileWatcher) {
        pidFileCleanup =
            java.lang.Runnable? {
            try {
                ...
                com.google.devtools.build.lib.server.ShutdownHooks.Companion.printStack(e)
            }
        }
    }
    ...
    fun runHooks() {
        ...
        val files: MutableList<com.google.devtools.build.lib.vfs.Path>?
        synchronized(this) {
            if (pidFileCleanup != null) {
                pidFileCleanup.run()
            }
            files = java.util.ArrayList<com.google.devtools.build.lib.vfs.Path>(filesToDeleteAtExit)
        }
        for (path in files!!) {
            try {
                path.delete()
    ...
    companion object {
        ...
        fun createAndRegister(): ShutdownHooks {
            val result: ShutdownHooks = com.google.devtools.build.lib.server.ShutdownHooks()
            java.lang.Runtime.getRuntime().addShutdownHook(java.lang.Thread(java.lang.Runnable { result.runHooks() }))
            return result
        }

        @kotlin.jvm.JvmStatic
        @com.google.common.annotations.VisibleForTesting
        fun createUnregistered(): ShutdownHooks {
            return com.google.devtools.build.lib.server.ShutdownHooks()
        }

        private fun printStack(e: IOException) {
            ...
            val err: java.io.StringWriter = java.io.StringWriter()
            val printErr: PrintWriter = PrintWriter(err)
            printErr.println("=======[BAZEL SERVER: ENCOUNTERED IO EXCEPTION]=======")
            e.printStackTrace(printErr)
            printErr.println("=====================================================")
            com.google.devtools.build.lib.server.ShutdownHooks.Companion.logger.atSevere().log("%s", err)
        }
    }
}
