 ...
 * 
 * 
 * Objects without children (as defined by the debugging protocol) should not be stored here.
 * 
 * 
 * Object identifiers apply only to a single thread, and aren't relevant to other threads.
 * 
 * 
 * This state is retained only while the thread is paused.
 */
internal class ThreadObjectMap {
    private val lastId = AtomicLong(1)
    private val objectMap: MutableMap<Long?, Any?> = ConcurrentHashMap<Long?, Any?>()
     ...
     * 
     * 
     * If called multiple times for a given object, a different identifier will be returned each
     * time. The object can be retrieved by all such identifiers.
     */
    fun registerValue(value: Any?): Long {
    ...
    fun getValue(identifier: Long): Any? {
