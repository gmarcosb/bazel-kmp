 ...
 * register test success/failure via [AnalysisTestResultInfo]. This helper registers the
 * appropriate test script simulating success or failure of the test.
 */
object AnalysisTestActionBuilder {
    /**
     ...
     */
    fun writeAnalysisTestAction(
        ruleContext: RuleContext, testResultInfo: AnalysisTestResultInfo
    ) {
        val escapedMessage: String =
            if (ruleContext.isDefaultExecGroupExecutingOnWindows())
                testResultInfo.getMessage()
                    .replace("%", "%%") // Prefix each character with \ (double-escaped; once in the string, once in the
