...
/** V1 cpu controller.  */
class LegacyCpu(private val path: Path) : LegacyController(), Cpu {
    @Throws(IOException::class)
    override fun child(name: String?): Cpu {
        return LegacyCpu(getChild(name))
    }
    ...
    override fun setCpus(cpus: Double) {
        val period: Long = Files.readString(path.resolve("cpu.cfs_period_us")).trim { it <= ' ' }.toLong()
        val quota = Math.round(cpus * period)
        Files.writeString(path.resolve("cpu.cfs_quota_us"), quota.toString())
    }

    @Throws(IOException::class)
    override fun getCpus(): Long {
        val quota: Long = Files.readString(path.resolve("cpu.cfs_quota_us")).trim { it <= ' ' }.toLong()
        val period: Long = Files.readString(path.resolve("cpu.cfs_period_us")).trim { it <= ' ' }.toLong()
        return quota / period
    }
}
