 ...
 * 
 * 
 * The position of the option is in category order, and within the priority category in index
 * order.
 */
class OptionPriority private constructor(
  @kotlin.jvm.JvmField private val priorityCategory: PriorityCategory,
  priorityIndices: com.google.common.collect.ImmutableList<Int?>
) : Comparable<OptionPriority?> {
    /**
     ...
     */
    private val priorityIndices: com.google.common.collect.ImmutableList<Int?>

    private var alreadyExpanded = false

    init {
        this.priorityIndices = priorityIndices
    }

    fun getPriorityCategory(): PriorityCategory {
        return priorityCategory
    }

    override fun compareTo(o: OptionPriority): Int {
        if (priorityCategory == o.priorityCategory) {
            var i = 0
            while (i < priorityIndices.size() && i < o.priorityIndices.size()) {
                if (priorityIndices.get(i) != o.priorityIndices.get(i)) {
            ...
            return java.lang.Integer.compare(priorityIndices.size(), o.priorityIndices.size())
        }
        return java.lang.Integer.compare(priorityCategory.ordinal(), o.priorityCategory.ordinal())
    }

    override fun equals(o: Any?): Boolean {
        if (o is OptionPriority) {
            val other = o
            return priorityCategory == other.priorityCategory
                    && priorityIndices == other.priorityIndices
        }
        return false
    }

    override fun hashCode(): Int {
        return java.util.Objects.hash(priorityCategory, priorityIndices)
    }

    override fun toString(): String {
        return java.lang.String.format("OptionPriority(%s,%s)", priorityCategory, priorityIndices)
    }
     ...
     * 
     * 
     * In general, new values for options can only override values with a lower or equal priority.
     * Option values provided in annotations in an options class are implicitly at the priority `DEFAULT`.
     * 
     * 
     * The ordering of the priorities is the source-code order. This is consistent with the
     * automatically generated `compareTo` method as specified by the Java Language
    ...
    enum class PriorityCategory {
        /**
         * The priority of values specified in the [Option] annotation. This should never be
         * specified in calls to [OptionsParser.parse].
         */
        DEFAULT,
    ...
    companion object {
        ...
        fun lowestOptionPriorityAtCategory(category: PriorityCategory): OptionPriority {
            return com.google.devtools.common.options.OptionPriority(
                category,
         ...
         * priorities.
         */
        fun nextOptionPriority(priority: OptionPriority): OptionPriority {
            val lastElementPosition: Int = priority.priorityIndices.size() - 1
            return com.google.devtools.common.options.OptionPriority(
        ...
        fun getChildPriority(parentPriority: OptionPriority): OptionPriority {
            if (parentPriority.alreadyExpanded) {
                throw com.google.devtools.common.options.OptionsParsingException("Tried to expand option too many times")
            }
            ...
            return com.google.devtools.common.options.OptionPriority(
                ...
                com.google.common.collect.ImmutableList.builder<Int?>().addAll(parentPriority.priorityIndices).add(0)
                    .build()
            )
        }
    }
