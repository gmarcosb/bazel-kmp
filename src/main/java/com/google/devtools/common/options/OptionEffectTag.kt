 ...
 * 
 * 
 * These will be used for filtering noise in long and complex command-lines, to help provide an
 ...
 * 
 * 
 * This file must be kept in sync with the matching proto. The information is duplicated to keep
 ...
 * 
 * 
 * These tags are used for flag filtering, so are consumed by tools that process bazel's output,
 * and for this reason must be kept backwards compatible until the build horizon has passed.
 * 
 * 
 *  * To add a new tag, add it here and to all flags it applies to. If you cannot do this in a
 ...
 *  * To remove a tag, remove it from all flags and mark it as deprecated for 6 months before
 * removing it entirely from the list below.
 *  * To change the intent of a tag (i.e. to tighten or loosen its definition), make sure that
 ...
 * 
 * 
 * ** Waived during the transition phase : The proto is not yet depended on externally and none
...
enum class OptionEffectTag(private val value: Int) {
     ...
     * 
     * 
     * Please do not use this value for new flags. This is meant to aid transition and for a very
     ...
     * 
     * 
     * Kept here for completeness and for deprecated flags. No new flag should set this tag.
     */
    NO_OP(1),
     ...
     * 
     * 
     * Yes, all options are technically inputs, but only options that affect inputs other
     ...
     * 
     * 
     * Even though many options implicitly change command line arguments because they change
