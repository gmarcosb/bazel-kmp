 ...
 * 
 * 
 * The syntax tree records the offsets within the file of all salient tokens, such as brackets
 ...
 * [.getStartLocation].
 */
abstract class Node internal constructor(locs: FileLocations?) {
    ...
    init {
        this.locs = Preconditions.checkNotNull<FileLocations>(locs)
    }
     ...
     * 
     * 
     * This function returns the canonical source code corresponding to a syntax tree. Generally,
     ...
     * 
     * 
     * The pretty-printed form of a syntax tree may be used as a proxy for equality in tests.
     * However, different trees may have the same printed form. In particular, [StarlarkFile]
     * includes comments that are not reflected in the string.
     */
    fun prettyPrint(): String {
        val buf = StringBuilder()
        NodePrinter(buf).printNode(this)
        return buf.toString()
    }
     ...
     * 
     * 
     * The output is not precisely specified; use [.prettyPrint] if you need more stable
    ...
    /**
     * Implements the double dispatch by calling into the node specific `visit` method of
     * the [NodeVisitor]
     * 
     * @param visitor the [NodeVisitor] instance to dispatch to.
     */
