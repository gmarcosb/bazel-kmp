...
/** Test for the [ActionExecutionStatusReporter] class.  */
@RunWith(JUnit4::class)
class ActionExecutionStatusReporterTest {
    private var events: EventCollectionApparatus? = null
    private var statusReporter: ActionExecutionStatusReporter? = null
    ...
    private fun mockAction(progressMessage: String?): Action {
        ...
        Mockito.`when`<T?>(action.getProgressMessage()).thenReturn(progressMessage)
        if (progressMessage == null) {
            Mockito.`when`<T?>(action.prettyPrint()).thenReturn("default message")
        }
        return action
    }
    ...
    fun initializeEventBus() {
        events = EventCollectionApparatus(com.google.devtools.build.lib.events.EventKind.ALL_EVENTS)
    ...
    private fun verifyNoOutput() {
        events.clear()
        statusReporter.showCurrentlyExecutingActions("")
        Truth.assertThat(events.collector()).isEmpty()
    }
    ...
    private fun verifyOutput(vararg lines: String?) {
        ...
        Truth.assertThat(
            ...
            .containsExactlyElementsIn(java.util.Arrays.asList<String?>(*lines))
            .inOrder()
    }

    @Throws(java.lang.Exception::class)
    private fun verifyWarningOutput(vararg lines: String?) {
    ...
    fun testCategories() {
        ...
        verifyOutput(
            "Still waiting for 4 jobs to complete:",
        ...
        verifyWarningOutput(
            "Still waiting for 3 jobs to complete:",
            "Running (remote):", "action3, 1 s",
            "Running (something else):", "action4, 0 s",
            "Scheduling:", "action2, 2 s",
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun testSingleAction() {
        val action: Action = mockAction("action1")
        verifyNoOutput()
        setPreparing(action)
    ...
    fun testDynamicUpdate() {
        ...
        eventBus.post(ScanningActionEvent(action))
    ...
    fun testGroups() {
        verifyNoOutput()
        val actions: MutableList<Action> = com.google.common.collect.ImmutableList.of<Action>(
        ...
        verifyOutput(
            "Still waiting for 6 jobs to complete:",
            "Scheduling:",
        ...
        for (a in actions) {
            setRunning(a, if (a.getProgressMessage().startsWith("remote")) "remote" else "something else")
        ...
        verifyOutput(
            "Still waiting for 5 jobs to complete:",
    ...
    fun testTruncation() {
        verifyNoOutput()
        val actions: MutableList<Action?> = java.util.ArrayList<Action?>()
        for (i in 1..100) {
            val a: Action = mockAction("a" + i)
            actions.add(a)
            setScheduling(a, "remote")
            clock.advanceMillis(1000)
        }
        verifyOutput(
            "Still waiting for 100 jobs to complete:", "Scheduling:",
        ...
        verifyOutput(
            "Still waiting for 100 jobs to complete:",
    ...
    private fun setScheduling(action: ActionExecutionMetadata?, strategy: String?) {
        eventBus.post(SchedulingActionEvent(action, strategy))
    }

    private fun setPreparing(action: Action?) {
        eventBus.post(ActionStartedEvent(action, 0))
    }

    private fun setRunning(action: ActionExecutionMetadata?, strategy: String?) {
        eventBus.post(RunningActionEvent(action, strategy))
    }
}
