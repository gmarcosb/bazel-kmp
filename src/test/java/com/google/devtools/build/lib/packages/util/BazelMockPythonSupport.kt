...
class BazelMockPythonSupport private constructor() : MockPythonSupport() {
    ...
    override fun setup(config: MockToolsConfig) {
        ...
        val path: PathFragment = PathFragment.create(runfiles.rlocation("rules_python/python/defs.bzl"))
        config.copyDirectory(
            path.getParentDirectory().getParentDirectory(), "rules_python_workspace", Int.Companion.MAX_VALUE, true
        )
    ...
    override fun getEmptyRunfilesSupplier(): Runfiles.EmptyFilesSupplier? {
        return BazelPyBuiltins.GET_INIT_PY_FILES
    }

    companion object {
        val INSTANCE: BazelMockPythonSupport = BazelMockPythonSupport()

        @Throws(IOException::class)
        private fun addTool(config: MockToolsConfig, toolRelativePath: String?) {
            config.create(
