...
import com.google.devtools.build.lib.buildtool.InstrumentationFilterSupport.getInstrumentedPrefix
import org.junit.Test

/** Unit test for Bazel coverage.  */
@RunWith(JUnit4::class)
class CoverageCommandUnitTest : BuildViewTestCase() {
    @Test
    @Throws(Exception::class)
    fun testGetInstrumentedPrefixJavatests() {
        ...
        val packageName: String? = getConfiguredTarget("//javatests/com/google/foo:l").getLabel()
            .getPackageName()
        Truth.assertThat(packageName).isEqualTo("javatests/com/google/foo") // No leading slashes
        assertThat(getInstrumentedPrefix(packageName)).isEqualTo("java/com/google/foo")
    }

    @Test
    fun testGetInstrumentedPrefix() {
