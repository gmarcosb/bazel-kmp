...
class LabelInternerIntegrationTest : SkyframeIntegrationTestBase() {
    private val labelInterner: LabelInterner = Label.getLabelInterner()
    ...
    fun setup() {
        ...
        Assume.assumeFalse(AnalysisMock.get().isThisBazel())
        assertThat(labelInterner).isNotNull()
    }

    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun labelInterner_noDuplicatesAndNoWeakInterned() {
        ...
        val allPackageTargetsLabelInstances: MutableList<Label> = java.util.ArrayList<Label>()
        val allRuleDepLabelInstances: MutableList<Label?> = java.util.ArrayList<Label?>()
        skyframeExecutor().getEvaluator().getInMemoryGraph().getValues().values().stream()
            .filter({ obj: Any? -> PackageValue::class.java.isInstance(obj) })
            .flatMap({ v -> (v as PackageValue).getPackage().getTargets().values().stream() })
            .forEach(
                { t ->
                    allPackageTargetsLabelInstances.add(t.getLabel())
                    if (t is Rule) {
                        allRuleDepLabelInstances.addAll(t.getLabels())
                    }
                })
        ...
        // instance.
        val strongInterner: com.google.common.collect.Interner<Label> = BlazeInterners.newStrongInterner<Label>()
        for (l in com.google.common.collect.Iterables.concat<Label?>(
            allPackageTargetsLabelInstances,
            allRuleDepLabelInstances
        )) {
            val canonical: Label = strongInterner.intern(l)
        ...
        for (l in allPackageTargetsLabelInstances) {
            val duplicate: Label? = Label.createUnvalidated(l.getPackageIdentifier(), l.name)
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun labelInterner_dirtyPackageStillPoolInternLabel() {
        write(
            "hello/BUILD",
            "load('@rules_cc//cc:cc_binary.bzl', 'cc_binary')",
        ...
        val packageKey: PackageIdentifier? = PackageIdentifier.createInMainRepo( /* name= */"hello")
        val nodeEntry: NodeEntry = graph.get( /* requestor= */null, Reason.OTHER, packageKey)
        assertThat(nodeEntry).isNotNull()

        val targetLabels: com.google.common.collect.ImmutableSet<Label?> =
            (nodeEntry.toValue() as PackageValue)
        ...
        targetLabels.forEach(
            java.util.function.Consumer { l: Label? ->
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun labelInterner_removeDirtyPackageStillWeakInternItsLabels() {
        write(
            "hello/BUILD",
            "load('@rules_cc//cc:cc_binary.bzl', 'cc_binary')",
            "cc_binary(name = 'foo', srcs = ['foo.cc'])"
     ...
     * 
     * 
     * Label interner can sometimes be disabled when blaze does not use [InMemoryGraph]. This
     * test case deliberately disables label interner and check identical label are always weak
     * interned.
     */
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun labelInterner_alwaysRespectWeakInternerWhenLabelInternerDisabled() {
        ...
        val preBuildLabel: Label? = Label.parseCanonical("//hello:foo")
        buildTarget("//hello:foo")
        ...
        val targetLabels: MutableSet<Label?>? =
            (nodeEntry.toValue() as PackageValue)
                .getPackage().getTargets().values().stream()
                .map(Target::getLabel)
                .collect(Collectors.toCollection(java.util.function.Supplier { com.google.common.collect.Sets.newIdentityHashSet() }))
        ...
        Truth.assertThat(targetLabels).contains(preBuildLabel)
        ...
        val postBuildLabel: Label? = Label.parseCanonical("//hello:foo")
