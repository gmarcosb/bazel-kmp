...
class RunfilesRepoMappingManifestTest : BuildViewTestCase() {
    ...
    fun setupBareBinaryRule() {
        registry.addModule(
            BzlmodTestUtil.createModuleKey("bare_rule", "1.0"), "module(name='bare_rule',version='1.0')"
        )
    ...
    @Throws(java.lang.Exception::class)
    private fun getRepoMappingManifestActionForTarget(label: String?): RepoMappingManifestAction {
        val action: Action? = getGeneratingAction(getRunfilesSupport(label).getRepoMappingManifest())
        assertThat(action).isInstanceOf(RepoMappingManifestAction::class.java)
        return action as RepoMappingManifestAction
    }
    ...
    private fun computeKey(action: RepoMappingManifestAction): String {
    ...
    @Throws(java.lang.Exception::class)
    private fun getRepoMappingManifestForTarget(label: String?): com.google.common.collect.ImmutableList<String?> {
    ...
    fun diamond() {
        ...
        val buildFiles: com.google.common.collect.ImmutableMap<String?, String?> =
        ...
        for (entry in buildFiles.entries) {
            scratch.overwriteFile(
                moduleRoot.getRelative(entry.key).getRelative("REPO.bazel").getPathString()
            )
            scratch.overwriteFile(
                moduleRoot.getRelative(entry.key).getRelative("BUILD").getPathString(),
                "load('@bare_rule//:defs.bzl', 'bare_binary')",
                entry.value
            )
        }
        ...
        Truth.assertThat(getRepoMappingManifestForTarget("//:aaa"))
            .containsExactly(
            ...
            .inOrder()
        Truth.assertThat(getRepoMappingManifestForTarget("@@ccc+//:ccc"))
    ...
    fun runfilesFromToolchain() {
        ...
        // manifest.
        registry.addModule(
            BzlmodTestUtil.createModuleKey("tooled_rule", "1.0"),
        ...
        Truth.assertThat(getRepoMappingManifestForTarget("//:tooled"))
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun actionRerunsOnRepoMappingChange_repoName() {
        scratch.overwriteFile(
            "MODULE.bazel",
            "module(name='aaa',version='1.0')",
            "bazel_dep(name='bare_rule',version='1.0')"
        ...
        val actionBeforeChange: RepoMappingManifestAction = getRepoMappingManifestActionForTarget("//:aaa")
        ...
        val actionAfterChange: RepoMappingManifestAction = getRepoMappingManifestActionForTarget("//:aaa")
        Truth.assertThat(computeKey(actionBeforeChange)).isNotEqualTo(computeKey(actionAfterChange))
    }

    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun actionRerunsOnRepoMappingChange_newEntry() {
        scratch.overwriteFile(
            "MODULE.bazel",
            "module(name='aaa',version='1.0')",
            "bazel_dep(name='bare_rule',version='1.0')"
    ...
    fun hasMappingForSymlinks() {
        ...
        registry.addModule(
            BzlmodTestUtil.createModuleKey("aaa", "1.0"),
            "module(name='aaa',version='1.0')",
        ...
        registry.addModule(
            BzlmodTestUtil.createModuleKey("symlinks", "1.0"),
        ...
        val runfilesSupport: RunfilesSupport = getRunfilesSupport("@aaa+//:aaa")
        val runfilesPaths: com.google.common.collect.ImmutableList<String?>? =
            runfilesSupport
                .getRunfiles()
                ...
                .collect(com.google.common.collect.ImmutableList.toImmutableList<E?>())
        Truth.assertThat(runfilesPaths)
            .containsAtLeast(
                "aaa+/aaa",
        ...
        Truth.assertThat(getRepoMappingManifestForTarget("@aaa+//:aaa"))
            .containsExactly( // @aaa+ contributes the top-level executable to runfiles.
                "aaa+,aaa,aaa+",  // The symlink is staged under the main repository's runfiles directory and aaa has a
                // repo mapping entry for it.
                "aaa+,my_module," + getRuleClassProvider().getRunfilesPrefix(),  // @symlinks+ appears as the first segment of a root symlink.
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun repoMappingOnFilesToRunProvider() {
        ...
        assertThat(BuildViewTestCase.Companion.getFilesToBuild(getConfiguredTarget("//:get_repo_mapping")).toList())
            .containsExactly(getRunfilesSupport("//:aaa").getRepoMappingManifest())
    }

    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun runfilesFromExtension(@TestParameter compactRepoMapping: Boolean) {
