...
class ProcMeminfoParserTest {
    private val scratch: FsApparatus = FsApparatus.Companion.newNative()

    @Test
    @Throws(Exception::class)
    fun memInfo() {
        val meminfoContent: String? =
        ...
        val meminfoFile: String? = scratch.file("test_meminfo", meminfoContent).getPathString()
        val memInfo: ProcMeminfoParser = ProcMeminfoParser(meminfoFile)
        ...
        Assert.assertThrows<T?>(
            ProcMeminfoParser.KeywordNotFoundException::class.java,
            ThrowingRunnable { memInfo.getRamKb("Bogus") })
        Assert.assertThrows<T?>(
            ProcMeminfoParser.KeywordNotFoundException::class.java,
            ThrowingRunnable { memInfo.getRamKb("Bogus2") })
    }

    @Test
    @Throws(Exception::class)
    fun testOldKernelFallback() {
        val meminfoContent: String? =
            StringUtilities.joinLines(
                "MemTotal:      3091732 kB",
                "Active:         636892 kB",
