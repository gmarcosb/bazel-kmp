...
class LibrariesToLinkCollectorTest : BuildViewTestCase() {
    /* TODO: Add an integration test (maybe in cc_integration_test.sh) when a modular toolchain config
  is available.*/
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun dynamicLink_siblingLayout_externalBinary_rpath() {
        ...
        analysisMock.ccSupport().setup(MockToolsConfig(rootDirectory.getChild("src")))
        ...
        val target: ConfiguredTarget = getConfiguredTarget("@@src+//test:foo")
        assertThat(target).isNotNull()
        val binary: Artifact = getExecutable(target)
        val linkAction: SpawnAction = getGeneratingAction(binary) as SpawnAction
        assertThat(linkAction).isNotNull()

        val workspace: String? = getTarget("//toolchain:toolchain").getPackageMetadata().workspaceName()
        val linkArgs: MutableList<String?>? = linkAction.getArguments()
        Truth.assertThat(linkArgs)
            .contains(
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun dynamicLink_siblingLayout_externalToolchain_rpath() {
        if (!analysisMock.isThisBazel) {
            return
        }
        scratch.appendFile(
        ...
        analysisMock.ccSupport().setup(MockToolsConfig(scratch.resolve("toolchain")))
        scratch.file(
            "toolchain/BUILD",
            "load('@rules_cc//cc/toolchains:cc_toolchain.bzl', 'cc_toolchain')",
            "load('@@//:cc_toolchain_config.bzl', 'cc_toolchain_config')",
