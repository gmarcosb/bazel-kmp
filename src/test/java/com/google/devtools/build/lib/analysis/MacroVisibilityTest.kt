 ...
 * 
 * 
 * This does *not* include tests of how the `visibility` attribute's value gets determined
 * and threaded through macros. See SymbolicMacroTest.java for that.
 */
@RunWith(TestParameterInjector::class)
class MacroVisibilityTest : BuildViewTestCase() {
    val defaultVisibility: String
        get() =// We're testing visibility. Avoid having to litter our test cases with `visibility=` attribute
     ...
     * `consumer` (expressed as a label string) to `dependency` (expressed as a label
     * string, or for `alias` targets, the string returned by [ ][AliasProvider.describeTargetWithAliases]).
     */
    /**
     * Convenience wrapper for [.visibilityErrorMessage] for when `dependency` is not an
     * `alias`.
     */
    private fun visibilityErrorMessage(
        consumer: String?, dependency: String?, dependencyIsAlias: Boolean = false
    ): String? {
    ...
    /**
     * Requests the evaluation of the configured target identified by the label `consumer`, and
     * asserts that the target analyzes successfully and has a dependency on the target identified by
     * the label `dependency`.
     * 
     * 
     * Does not work when `dependency` is an `alias` target.
     */
    @Throws(java.lang.Exception::class)
    private fun assertVisibilityPermits(consumer: String, dependency: String?) {
        val consumerTarget: ConfiguredTarget? = getConfiguredTarget(consumer)
        ...
        Truth.assertWithMessage(
            ...
            .that(consumerTarget)
            .isNotNull()
        Truth.assertWithMessage("%s does not have a dependency on %s", consumer, dependency)
     ...
     * 
     * 
     * Removes the `failFastHandler`.
     */
    /**
     * Convenience wrapper for [.assertVisibilityDisallows] for when `dependency` is not
     * an `alias`.
     */
    @Throws(java.lang.Exception::class)
    private fun assertVisibilityDisallows(
        consumer: String, dependency: String?, dependencyIsAlias: Boolean = false
    ) {
    ...
    /**
     * Creates a package `//common` defining several `simple_rule` targets with the given
     * names and visibility declarations.
     * 
     * 
     * The argument list must have even length, and contains pairs of (unquoted) target names and
     * Starlark expression strings that evaluate to visibility lists (e.g. pass the string `"[]"` for no visibility).
     */
    @Throws(java.lang.Exception::class)
    private fun defineCommonPackageWithSimpleTargets(vararg targetNamesAndVisibilities: String?) {
        com.google.common.base.Preconditions.checkArgument(targetNamesAndVisibilities.size % 2 == 0)
        ...
        var i = 0
        while (i < targetNamesAndVisibilities.size) {
            s.append(
                String.format(
                    """
              simple_rule(
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun buildFileAccessToMacroTargetsIsControlled() {
        defineSimpleRule()
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun macroAccessToBuildFileTargetsIsControlled() {
        defineSimpleRule()
        scratch.file("lib/BUILD")
        scratch.file(
            "lib/macro.bzl",
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun macroAccessToOtherMacrosInSameBuildFileIsControlled() {
        defineSimpleRule()
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun siblingsInSameMacroCanSeeEachOther() {
        defineSimpleRule()
        scratch.file("lib/BUILD")
        scratch.file(
            "lib/macro.bzl",
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun buildFileTargetIsVisibleToTargetsDefinedByMacroFromSamePackage() {
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun macroDefinitionLocationIsDeterminedByExportAssignment() {
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun locationOfTargetDeclarationIsInnermostMacro() {
    ...
    @Throws(java.lang.Exception::class)
    fun buildFileCanDelegateVisibilityPrivilegesToMacro(depIsConfigurable: Boolean) {
        defineSimpleRule()
        ...
        scratch.file(
            ...
            """.trimIndent(),
                if (depIsConfigurable) "True" else "False"
            )
        )
        scratch.file(
            "pkg/BUILD",
    ...
    /**
     * Defines a macro `name` whose body calls symbol `wraps` to instantiate its main
     * target or submacro, passing along `attrExpr`.
     * 
     * 
     * The new macro is declared in `//${pkg}:macro.bzl`. It takes an optional `dep`
     * attribute.
     * 
     * @param pkg the package where the macro is defined
     * @param name the macro being introduced, e.g. "my_macro", which would be loaded from `//${pkg}:macro.bzl`
     * @param wraps the rule or macro to be called by this macro, in `bzlLabel%symbol` form,
     * e.g. "//some_pkg:defs.bzl%my_rule"
     * @param attrExpr an attribute argument expression for the call site of `wraps`, e.g.
     * "some_attr = dep"
     */
    @Throws(java.lang.Exception::class)
    private fun defineWrappingMacro(pkg: String?, name: String?, wraps: String, attrExpr: String?) {
        val i: Int = wraps.indexOf("%")
        com.google.common.base.Preconditions.checkArgument(i != -1)
        val bzlToLoad: String = wraps.substring(0, i)
        val symbolName: String = wraps.substring(i + 1)
    ...
    /**
     * Defines a macro `name` whose body calls `wraps` to instantiate its main target or
     * submacro, forwarding along the `dep` attribute (if supplied) unchanged.
     * 
     * 
     * The new macro is declared in `//<name>:macro.bzl`.
     * 
     * @param name the macro being introduced, e.g. "my_macro", which would be loaded from `//my_macro:macro.bzl`
     * @param wraps the rule or macro to be called by this macro, in `bzlLabel%symbol` form,
     * e.g. "//some_pkg:defs.bzl%my_rule"
     */
    @Throws(java.lang.Exception::class)
    private fun defineWrappingMacroWithSameDep(name: String?, wraps: String) {
        defineWrappingMacro(name, name, wraps, "dep = dep")
    }

    /**
     * Defines a macro `name` whose body calls `wraps` to instantiate its main target or
     * submacro, passing `hardcodedDep` as the value of the `dep` attribute.
     * 
     * 
     * The new macro is declared in `//<name>:macro.bzl`. It takes an optional `dep`
     * attribute itself, which gets ignored.
     * 
     * @param name the macro being introduced, e.g. "my_macro", which would be loaded from `//my_macro:macro.bzl`
     * @param wraps the rule or macro to be called by this macro, in `bzlLabel%symbol` form,
     * e.g. "//some_pkg:defs.bzl%my_rule"
     * @param hardcodedDep a label string to pass as `dep = <hardcodedDep>` at the call site of
     * `wraps`, e.g. "//some_pkg:my_target"
     */
    @Throws(java.lang.Exception::class)
    private fun defineWrappingMacroWithHardcodedDep(name: String?, wraps: String, hardcodedDep: String?) {
        defineWrappingMacro(name, name, wraps, String.format("dep = \"%s\"", hardcodedDep))
    }

    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun outerMacroCanDelegateVisibilityPrivilegesToSubmacro() {
        defineSimpleRule()
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun delegationCanBeTransitiveThroughAnIntermediaryMacro() {
        defineSimpleRule()
        defineCommonPackageWithSimpleTargets("vis_to_pkg", "['//pkg:__pkg__']")
        defineWrappingMacroWithSameDep("inner", "//rules:simple_rule.bzl%simple_rule")
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun callerPermissionIsCheckedEvenIfCalleeHasItsOwnPermission() {
        defineSimpleRule()
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun noDelegationIfCallerCannotSeeDep() {
        defineSimpleRule()
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun noDelegationIfCallerDoesNotPassInTarget() {
        defineSimpleRule()
        defineCommonPackageWithSimpleTargets( // Read: "v2[P|M]" -> "visible to [package|macro]"
            "v2P_hardcoded",
            "['//pkg:__pkg__']",
            "v2P_implicitdep",
            "['//pkg:__pkg__']",
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun delegationCannotSkipOverAGap() {
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun delegationDoesNotFollowAliases() {
        ...
        assertVisibilityDisallows(
            "//pkg:foo",
            "alias '//common:aliasof_vis_to_pkg' referring to target '//common:vis_to_pkg'",  /* dependencyIsAlias= */
            true
        )
    }
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun packageDefaultVisibilityDoesNotPropagateToInsideMacro() {
        defineSimpleRule()
        defineWrappingMacroWithSameDep("my_macro", "//rules:simple_rule.bzl%simple_rule")
        defineWrappingMacroWithSameDep("inner", "//rules:simple_rule.bzl%simple_rule")
        defineWrappingMacroWithSameDep("outer", "//inner:macro.bzl%inner")
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun finalizerTargetsCanSeeTargetsVisibleToBuildFileOrFinalizerMacroClassDefinition() {
        defineSimpleRule()
