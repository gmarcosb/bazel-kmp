...
object BazelMockAndroidSupport {
    @Throws(IOException::class)
    fun setupNdk(config: MockToolsConfig?) {
        Crosstool(config, "android/crosstool", Label.parseCanonicalUnchecked("//android/crosstool"))
            .setCcToolchainFile(
                ResourceLoader.readFromResources(
                    "com/google/devtools/build/lib/packages/util/mock/android_cc_toolchain_config.bzl"
                )
            )
            .setToolchainConfigs(ImmutableList.of<CcToolchainConfig?>(x86Config()!!.build(), armeabiV7a()!!.build()))
            .setSupportedArchs(ImmutableList.of<String?>("x86", "armeabi-v7a"))
            .setSupportsHeaderParsing(false)
            .write()
    }

    fun x86Config(): Crosstool.CcToolchainConfig.Builder? {
    ...
    fun armeabiV7a(): Crosstool.CcToolchainConfig.Builder? {
        return CcToolchainConfig.Companion.builder()
            .withCpu("armeabi-v7a")
            .withCompiler("gcc")
