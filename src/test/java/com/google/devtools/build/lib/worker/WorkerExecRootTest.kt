...
/** Tests for [WorkerExecRoot].  */
@RunWith(JUnit4::class)
class WorkerExecRootTest {
    /** The global execroot directory, where real files live.  */
    private var execRoot: Path? = null

    /** The `workDir` directory of the worker. This is the CWD of the worker process.  */
    ...
    fun cleanFileSystem() {
        ...
        assertThat(
            FileSystemUtils.readContent(workDir.getRelative("worker.sh"), java.nio.charset.Charset.defaultCharset())
        )
            .isEqualTo("#!/bin/sh")
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun createsAndCleansInputSymlinks() {
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun workspaceFilesAreNotDeleted() {
        ...
        val neededWorkspaceFile: Path = execRoot.getRelative("needed_file")
        val otherWorkspaceFile: Path = execRoot.getRelative("other_file")
        ...
        assertThat(FileSystemUtils.readContent(neededWorkspaceFile, java.nio.charset.Charset.defaultCharset()))
            .isEqualTo("needed workspace content")
        assertThat(FileSystemUtils.readContent(otherWorkspaceFile, java.nio.charset.Charset.defaultCharset()))
            .isEqualTo("other workspace content")
    }

    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun recreatesEmptyFiles() {
        ...
        val workerExecRoot: WorkerExecRoot =
            WorkerExecRoot(sandboxHelper.workDir, com.google.common.collect.ImmutableList.of<E?>(), false)
        workerExecRoot.createFileSystem(
            sandboxHelper.getWorkerFiles(),
            sandboxHelper.getSandboxInputs(),
            sandboxHelper.getSandboxOutputs(),
        ...
        assertThat(
            FileSystemUtils.readContent(workDir.getRelative("some_file"), java.nio.charset.Charset.defaultCharset())
        )
            .isEmpty()
    }
    ...
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun createsAndDeletesSiblingExternalRepoFiles() {
        ...
        val fooRepoDir: Path = execRoot.getRelative("external_dir/foo")
        val input1: Path = fooRepoDir.getRelative("bar/input1")
        val input2: Path = fooRepoDir.getRelative("input2")
        val random: Path = fooRepoDir.getRelative("bar/random")
