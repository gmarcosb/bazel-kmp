...
/**
 * Abstract class for setting up the blackbox test environment, returns [BlackBoxTestContext]
 ...
 * 
 * 
 * The single instance of this class can be used for several tests. Each test should call [ ][.prepareEnvironment] to prepare environment and obtain the test context.
 * 
 * 
 * See [com.google.devtools.build.lib.blackbox.junit.AbstractBlackBoxTest]
 */
abstract class BlackBoxTestEnvironment {
    /**
     * Executor service for reading stdout and stderr streams of the process. Has exactly two threads
     * since there are two streams.
     */
    private var executorService: ExecutorService? = MoreExecutors.getExitingExecutorService(
        Executors.newFixedThreadPool(2) as ThreadPoolExecutor,
        1, TimeUnit.SECONDS
    )

    @Throws(Exception::class)
    protected abstract fun prepareEnvironment(
        testName: String?, tools: ImmutableList<ToolsSetup?>?, executorService: ExecutorService?
    ): BlackBoxTestContext?
     ...
     * @param tools the list of all tools setup classes [ToolsSetup] that should be called for
     * the test
     * @return [BlackBoxTestContext] test context
     * @throws Exception if tools setup fails
     */
    @Throws(Exception::class)
    fun prepareEnvironment(testName: String?, tools: ImmutableList<ToolsSetup?>?): BlackBoxTestContext? {
