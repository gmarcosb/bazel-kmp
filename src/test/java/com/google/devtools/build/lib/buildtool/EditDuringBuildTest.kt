 ...
 * 
 * 
 * This shows that the problem described in bug #730818 and discussed in the reviewlog for OCL
 ...
 * 
 * 
 * However, in the case of C++ compilation, since we don't know the inputs ahead of time, must
 * stat the input files after execution, which leads to the race condition. The long term fix is to
 * use a precise C++ dependency scanner and eliminate support for the `!inputsDiscovered()`
...
class EditDuringBuildTest : BuildIntegrationTestCase() {
    @org.junit.Test
    @Throws(java.lang.Exception::class)
    fun testEditDuringBuild() {
        val `in`: Path = write("edit/in", "line1")
        ...
        var out = buildAndReadOutputFile()
        Truth.assertThat(out).isEqualTo("line1\n")
    ...
    @Throws(java.lang.Exception::class)
    private fun buildAndReadOutputFile(): String? {
        buildTarget("//edit:out")
        return readContentAsLatin1String(com.google.common.collect.Iterables.getOnlyElement<Artifact?>(getArtifacts("//edit:out")))
    }
}
